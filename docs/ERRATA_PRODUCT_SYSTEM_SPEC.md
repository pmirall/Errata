# ERRATA — Product, System & Software Design Specification
...
**Goal:** Build Errata end-to-end from the existing project, replacing obsolete browser-game functionality and implementing the complete product loop.

---

# 0. Executive mandate

Errata is a physical virtual-pet game built around an ESP32.

A Bug is a small digital creature — a "computer bug" — trapped in wireless networks. The player cares for one active Bug, carries the device around, discovers events through nearby Wi-Fi networks, captures and evolves Bugs, collects them in a box, and interacts with other Errata devices through battle, trading, and breeding.
...
Do not reproduce Pokémon mechanically. Errata should have its own terminology, progression, creature design language, and gameplay identity.
...
The player maintains one selected Bug.
...
Only the selected Bug requires active care.

Bugs stored in the Box do not require manual care. Their passive state improves slowly while stored, up to healthy limits, over approximately 24 hours.
...
The player carries the Errata.

Nearby Wi-Fi networks become the "web" in which Bugs live.
...
Different Bug species have different spawn rates.
...
- capture them;
- evolve them;
- breed compatible Bugs;
- trade them;
- store up to 10 Bugs in the Box.

The selected Bug is the one that is actively carried and cared for.
...
Each player brings up to **3 Bugs**.

Battle is:
- 1v1 active Bug;
- turn-based;
- four attacks per Bug;
...
The player can eventually create a custom Bug from a mobile interface.
...
The system validates the generated Bug against the same balance rules used by built-in Bugs.
...
Errata must be implemented as an explicit state machine.
...
The selected Bug is the active one.
...
A Bug can only occupy one location.
...
Each Bug should contain at minimum:
...
They should have an original Errata terminology tied to computing/network concepts.
...
```text
Player A: up to 3 Bugs
Player B: up to 3 Bugs
```
...
The same validator used for custom Bugs should be used for exchanged Bugs.

---

# 16. Trading

When two Errata devices connect:
...
- both devices retain their original Bug.
...
Only compatible Bugs may breed.
...
Breeding should produce a valid Bug instance using controlled inheritance.
...
Bugs can evolve according to level and species-specific rules.
...
**At least 60 total Bug creatures.**
...
- the encounter must never delete the active Bug.
...
Bugs should be inconveniently unhappy at worst, not permanently destroyed.
...
The ESP32 must independently validate uploaded Bugs.
...
A custom Bug must fit the same competitive balance envelope as built-in Bugs.
...
When another Errata is discovered:
...
Each Errata device needs a local identity.
...
Bugs are computer bugs and may occasionally become "corrupted".
...
Corruption must never permanently destroy a Bug.
...
A Bug instance may have a small number of traits derived from usage.
...
Errata should reward:
- exploration;
- experimentation;
- collection;
- social play;
- caring for a favorite Bug.
...
- [ ] Box supports 10 Bugs.
- [ ] Active Bug can be selected.
...
- [ ] Generated Bugs remain balanced.
...
4. **Do not build cloud infrastructure. Errata is offline-first.**
...
12. **Never trust another Errata to provide legal game state.**
13. **Make trades atomic.**
14. **Never silently destroy a Bug.**
15. **Never make care mechanics require constant attention.**
16. **Avoid Pokémon-level complexity.**
17. **Do not let custom Bugs become pay-to-win / stat-cheat machines.**
...
Errata succeeds if the player can forget about it for a while, pick it up, press two buttons, smile, and think:
...
# Appendix C — Example custom Bug validation
...
The finished Errata should be a coherent, self-contained physical game system:
...
> A Bug is not a menu item. It is a little digital creature with a life that happens while the player carries the device through the real world.
