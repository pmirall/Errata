...
| The volume ladder is strictly decreasing, never silent and never past a square wave, and the pitch does not move with it | `tests/test_audio.cpp`, on the duty the engine hands the sink, plus `tests/test_sound.cpp` for the persisted half | **that any of the three levels is audible, or that they sound different from each other.** `kVolDuty` is 50/16/5 % and nobody has heard it: no piezo has been fitted to a board this firmware has run on. Expect to retune the middle step. Also unproven: that `ledcWrite()` after `ledcWriteTone()` actually narrows the pulse on real silicon rather than being overwritten by the tone call |
