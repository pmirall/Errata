# PEBBLEBOL — Hardware Decisions & Battery Optimization Specification

**Status:** V1 hardware baseline  
**Purpose:** Define the intentionally minimal hardware architecture for Pebblebol and the firmware rules required to maximize battery life.  
...
Pebblebol is deliberately designed as a **small, low-cost, low-power physical game device**.
...
The controller provides the functionality needed by Pebblebol:
...
Never keep the OLED continuously refreshing just so that the Pebble appears "alive".
...
Pebblebol should have four conceptual power states:
...
The actual Pebblebol current will be higher because the complete system includes the development board, regulator, OLED and other hardware.
...
This means Pebblebol can safely sleep for hours without running its CPU.
...
```text
"Pebble has been carried a lot"
```

not:

```text
"Pebble traveled exactly 2.3847 km"
```
...
> **Pebblebol is fun with extremely simple hardware.**
...
The Pebblebol V1 hardware is intentionally:
...
Pebblebol should behave like a tiny toy that is **mostly asleep, wakes instantly when touched, briefly activates the expensive peripherals when needed, and returns to sleep as soon as possible.**
...
That architecture is the primary battery optimization strategy for Pebblebol V1.
