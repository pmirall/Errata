...
a WPA access point. It cannot be had together with the join QR, and the arithmetic is
...
without reporting it — a valid-looking `WIFI:` string with a mangled passphrase.

Spec §39 asks for exactly what is shipped: the compact local URL, and *"do not put secrets
into the QR beyond what is necessary. The PIN remains the user-facing authorization layer."*
What makes the open network defensible is measured rather than asserted — the portal exists
only while the CREATOR screen is open, it tears itself down after `creator_idle_s` (300 s),
and P8-C1's five-failure / 60 s lockout bounds an in-range attacker to about **nine guesses
per portal session out of 10,000**, or ~0.09 %.

**Left open for the owner, and it is a bench call, not a code change:** if WPA is still
wanted, either the CREATOR layout is redesigned so a 70 px symbol fits, or the join symbol is
dropped and the passphrase is printed as text beside the URL symbol. Whether a 62 px version-2
symbol scans from a phone at arm's length on a 0.96" panel is itself unobserved here.
