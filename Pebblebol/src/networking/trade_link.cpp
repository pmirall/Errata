// =============================================================================
//  PEBBLEBOL - networking/trade_link.cpp
...
// -----------------------------------------------------------------------------
static VReject tw_decode(const uint8_t rec[TR_WIRE_BYTES], PebbleInstance& out)
{
...
// The CRC is then recomputed over exactly the span PBW_CRC_BYTES names, so
...
{
  rec[PBW_OFF_ID + 0] = (uint8_t)(id & 0xFFu);
  rec[PBW_OFF_ID + 1] = (uint8_t)((id >> 8) & 0xFFu);
  rec[PBW_OFF_ID + 2] = (uint8_t)((id >> 16) & 0xFFu);
  rec[PBW_OFF_ID + 3] = (uint8_t)((id >> 24) & 0xFFu);
  const uint16_t crc = crc16_ccitt(rec, (size_t)PBW_CRC_BYTES);
  rec[PBW_OFF_CRC + 0] = (uint8_t)(crc & 0xFFu);
  rec[PBW_OFF_CRC + 1] = (uint8_t)((crc >> 8) & 0xFFu);
}
                     ...
                     const TradeHooks& hooks)
{
  ...
  memcpy(tl.out_rec, s.my_rec[0], (size_t)PBW_BYTES);
  s.tl = &tl;
}
...
static void send_offer(Session& s, bool retx)
{
  ...
  memcpy(m.p.toffer.rec, s.tl->out_rec, (size_t)PBW_BYTES);
  session_emit(s, m);
}
...
static void on_offer(Session& s, const ProtoMsg& m)
{
  ...
  if (tl.have_in != 0u) {
    ...
    // consent a player is about to give names one specific pair of Pebbles.
    if (crc != tl.in_crc ||
        memcmp(tl.in_rec, m.p.toffer.rec, (size_t)PBW_BYTES) != 0) {
  ...
  memcpy(tl.in_rec, m.p.toffer.rec, (size_t)PBW_BYTES);
  tl.have_in = 1u;
  tl.in_crc  = crc;
  fix_pair_crc(s);

  // THE ONE VALIDATOR, on the wire path, exactly where protocol.h says it runs:
  // pbw_decode() IS validate_pebble() in front of a decode, and its answer is
...
// The one place a Pebble moves. Reached only with BOTH confirms in hand.
