                 ...
                 uint32_t now_epoch, CaptureReport& out)
{
  ...
  // THE ONE REJECT A CONSTRUCTED PEBBLE CAN CARRY, refused before anything is
  ...
  // the tree there).
  const uint8_t slot = box_new_pebble(enc.species_id, enc.level, (uint8_t)ORIGIN_WILD,
  ...
  const uint8_t vr = (p != nullptr) ? (uint8_t)validate_pebble(*p)
