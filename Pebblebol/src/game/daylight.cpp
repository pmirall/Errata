...
static uint16_t lerp_min(uint16_t a, uint16_t b, uint16_t pos, uint16_t span)
{
  ...
  if (v < 0) v = 0;
  if (v > (int32_t)(PB_MINUTES_PER_DAY - 1u)) v = (int32_t)(PB_MINUTES_PER_DAY - 1u);
  return (uint16_t)v;
}
...
static uint16_t sample(const uint16_t* table, uint16_t day_of_year)
{
  ...
  const uint16_t last  = DAYLIGHT_ANCHOR_DOY[PB_DAYLIGHT_MONTHS - 1];
  ...
  if (d < first || d >= last) {
    ...
    return lerp_min(table[PB_DAYLIGHT_MONTHS - 1], table[0], pos, span);
  }

  for (uint8_t m = 0; m + 1u < (uint8_t)PB_DAYLIGHT_MONTHS; ++m) {
  ...
  return table[PB_DAYLIGHT_MONTHS - 1];   // unreachable; the loop covers it
}
...
uint16_t daylight_bedtime_min(uint16_t day_of_year)
{
  ...
  return (uint16_t)(s % PB_MINUTES_PER_DAY);
}

uint8_t daylight_is_night(uint16_t day_of_year, uint16_t minute_of_day)
{
  const uint16_t m    = (uint16_t)(minute_of_day % PB_MINUTES_PER_DAY);
