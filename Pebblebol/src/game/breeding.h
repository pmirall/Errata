...
//  every one of those through game/box.cpp's box_new_pebble(), the tree's one
//  Pebble constructor, and then validate_pebble() before the slot is kept.
//
// -----------------------------------------------------------------------------
...
//  game/pebble.h folds a gene through gvar(v) = v * 3 / 16, so 0..15 becomes
//  0..2 whatever any number of generations does, PEBBLE_GENOME_VAR_MAX is 2,
//  and game/validate.h's rule (b) says a range check on a gene is a test that
//  cannot fail. Nothing this module does could raise a bred Pebble's atk, def
...
//  esp_random(), no I/O.
// =============================================================================
#ifndef PB_GAME_BREEDING_H
#define PB_GAME_BREEDING_H
...
//  parents of a refused pair are perfectly legal Pebbles.
//
...
enum BreedReject : uint8_t {
  ...
  BRD_SAME_UNIT,          // one Pebble cannot breed with itself (equal ids)
  BRD_UNKNOWN_SPECIES,    // species_get() answers nullptr for a parent. CHECKED
                          ...
                          // validate_pebble() answers VR_UNKNOWN_SPECIES first.
  BRD_INVALID_PARENT,     // validate_pebble() refused a parent
  ...
  BRD_INVALID_CHILD,      // the offspring failed validate_pebble(): a bug HERE,
...
//  two devices file two different Pebbles into two different Boxes.
// -----------------------------------------------------------------------------
struct BreedPlan {
