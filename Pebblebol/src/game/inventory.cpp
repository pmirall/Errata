...
//  THE KLASS ARMS
// -----------------------------------------------------------------------------
static uint8_t use_xp_candy(uint8_t item_id, PebbleInstance& p, ItemEffect& eff)
{
  const uint16_t amount = item_xp_value(item_id);
  if (amount == 0u) return (uint8_t)IU_NO_EFFECT;
  // "Did anything happen" is asked of the PEBBLE and not of xp_add()'s return
...
static uint8_t use_care(const ItemDef& it, PebbleInstance& p, uint32_t now_epoch,
                        uint8_t cal, ItemEffect& eff)
{
  ...
  for (uint8_t i = 0; i < (uint8_t)PB_CARE_COUNT; ++i) {
    ...
    if (p.care[i] >= (int32_t)PB_CARE_MILLI_MAX) continue;   // already full
  ...
  // A well Pebble at full health does not waste the item.
...
static uint8_t use_evolution(const ItemDef& it, PebbleInstance& p, ItemEffect& eff)
{
                ...
                uint32_t now_epoch, uint8_t cal, ItemEffect& eff)
{
  ...
  const bool needs_pebble = (it->klass == (uint8_t)ITEM_KLASS_XP_CANDY ||
                             it->klass == (uint8_t)ITEM_KLASS_CARE ||
                             it->klass == (uint8_t)ITEM_KLASS_EVOLUTION);
  if (needs_pebble && (target == nullptr || pebble_is_empty(*target)))
