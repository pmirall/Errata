...
TradeReject trade_offer_check(uint8_t slot, uint16_t quarantine_mask)
{
  ...
  // THE LAST PEBBLE ON THE DEVICE IS ALREADY PROTECTED, by the line above and
  ...
  if (validate_pebble(*p) != VR_OK) return fail(TDR_INVALID);
  return fail(TDR_OK);
}
                               ...
                               uint32_t local_device_id, uint32_t peer_device_id)
{
  ...
  if (validate_pebble(incoming) != VR_OK) return fail(TDR_PEER_INVALID);
  // THE GOD-TAINT GATE (game/taint.h). Evaluated on OUR outgoing Pebble against
  ...
  // A peer that sends a Pebble whose id we already hold would break invariant
                                   ...
                                   const TradeStore& store)
{
  ...
  if (!decoded) {
    // The record will not decode. If the outgoing Pebble is still here, nothing
    // has been destroyed and the honest answer is to undo; if it is already
    // gone, one Pebble has been lost and no code in this file can bring it back.
  ...
  if (out_slot != (uint8_t)BOX_SLOT_NONE) {
    ...
    // Pebble at all, so this is DEFENSIVE - the input here is a persisted blob
    // and a blob is not a promise. The incoming Pebble becomes active below.
  ...
  // --- B2: the incoming Pebble arrives --------------------------------------
  ...
  if (!box_id_in_use(in.id)) {
    ...
    // A FULL BOX WITH THE OUTGOING PEBBLE ALREADY GONE. B1 frees a slot on
    ...
    const PebbleInstance* filed = box_peek(slot);
