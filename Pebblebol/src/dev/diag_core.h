...
//  TWO markers: the genome bit GN_TAINT_SH and PebbleInstance.flags's
...
//  box_new_pebble() memsets the record and takes the genome as given, so a
//  `spawn` written the obvious way MINTS A CLEAN PEBBLE INSIDE GOD MODE, which
...
// =============================================================================
#ifndef PB_DIAG_CORE_H
#define PB_DIAG_CORE_H
...
#define DCF_TAINTS    0x04u   // ...a PebbleInstance, so it must mark it (taint.h)
...
// so the CRC agrees; PebbleInstance.crc16 is sealed by persistence on the way
...
enum DiagResult : uint8_t {
  ...
  DGR_ERR_NO_PET,     // there is no active Pebble
...
#define DGCOMMIT_ACTIVE   0x02u   // the active PebbleInstance
...
struct DiagCtx {
  ...
  uint8_t    have_sim;      // 1 when sim_bind() has run on the active Pebble
};
