...
#define BT_MENU_ROWS     ((uint8_t)(ER_MOVE_COUNT + 1u))       // 4 attacks + CAMBIAR
#define BT_MENU_SWITCH   ((uint8_t)ER_MOVE_COUNT)              // that last row
...
static char s_name_utf8[2][ER_NAME_DRAW_CAP];

static const char* combatant_name(uint8_t side, uint8_t slot) {
  if (side > 1u || slot >= (uint8_t)BATTLE_TEAM_MAX) return "";
  const BugInstance& m = s_setup.member[side][slot];
  char* out = s_name_utf8[side];
  out[0] = '\0';
  if (m.nickname[0] != '\0') {
    (void)u8_cat_latin1(out, (uint16_t)ER_NAME_DRAW_CAP, m.nickname);
  } else {
    const char* sp = pet_species_name(m.species_id);
    (void)u8_cat(out, (uint16_t)ER_NAME_DRAW_CAP, sp ? sp : "");
  }
  return out;
}
...
static void make_member(BugInstance& p, uint8_t species_id, uint8_t level,
  ...
  // The three genes bug_derive_stats() reads, at their modal value: 8 folds
  // to +1 through bug_genome_var(), which is the (1,1,1) roll the whole
...
// One of the player's Bugs, COPIED out of the Box. Nothing here writes back,
// which is the whole of the "an interrupted battle cannot corrupt the Box"
// argument.
bool battle_copy_from_box(BugInstance& out, uint8_t slot) {
  const BugInstance* src = box_peek(slot);
  if (src == nullptr) return false;
  out = *src;
  const SpeciesDef* sp = species_get(out.species_id);
  if (sp == nullptr) return false;
  if (out.level == 0u) out.level = 1u;
  // A stored Bug from a v1 save can carry moves[] = {0,0,0,0}
  ...
  // carrying hp_cur in - means a Bug at 1 HP cannot practise at all, and
  // means the fight would have to write the damage back to be honest about it.
  BugStats st;
  bug_derive_stats(*sp, out.level, out.genome, st);
...
// ids are whatever box_new_bug() minted, so the synthetic ones are checked
...
static bool menu_row_legal(uint8_t r) {
  if (r < (uint8_t)ER_MOVE_COUNT) return attack_legal(r);
...
static void build_message(void) {
  ...
  switch (e->kind) {
    ...
    case RLE_HIT: {
      ...
      const AttackDef* a = (c && e->a < (uint8_t)ER_MOVE_COUNT)
...
static void enter_beat(uint16_t i) {
  ...
  // omission: the beat immediately before it is the player's last Bug going
...
uint8_t battle_screen_cursor_reject(void) {
  ...
  if (s_mode == BTM_MENU) {
    if (s_cur < (uint8_t)ER_MOVE_COUNT) {
      const BattleAction a = { (uint8_t)BACT_ATTACK, s_cur };
