...
//  SHAPE
// -----------------------------------------------------------------------------
// 20 -> ER_NAME_DRAW_CAP at P10-C4: a peer name is up to NAME_MAX_LEN Latin-1
// characters and therefore up to twice that many UTF-8 bytes once drawn.
#define LK_ROW_CAP        ((uint8_t)ER_NAME_DRAW_CAP)
#define LK_VAL_CAP         8
...
#define LK_NAME_CAP       ((uint8_t)ER_NAME_DRAW_CAP)
...
//  not a Errata in the room (networking/discovery.h). The unqualified ones
...
//
//  Up to BATTLE_TEAM_MAX Box slots, the ACTIVE one first so the Bug the
//  player is carrying leads, then slot order. A slot the BOX pre-selected
//  (link_arm_intent) leads instead, which is what "pre-selects that Bug"
...
//  the result and its verdict is final; a Bug this device cannot make legal
//  is refused by name and never offered.
// -----------------------------------------------------------------------------
static uint8_t build_team(BugInstance* out, uint8_t cap) {
  uint8_t n = 0;
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
//  THE ONE BUG THIS DEVICE OFFERS IN A TRADE
//
...
//  Bug is not for sale, a quarantined one may not enter a trade, an object
...
static uint8_t pick_trade_slot(uint8_t& why) {
  ...
  if (s_intent_slot < (uint8_t)BOX_SLOTS) {
    ...
    // different Bug: the player chose that one.
    return (uint8_t)BOX_SLOT_NONE;
  }
  // KEEP THE MOST INFORMATIVE REFUSAL, and the order matters because slot 0 is
  // usually the ACTIVE one: "guarda ese Bug primero" is the right answer for
  // a player whose only other Bugs are fine, and the WRONG one for a player
  // whose other Bugs are all quarantined. So an empty slot's reason is
...
static bool open_session(void) {
  ...
  // friends in the same room is not matchmaking: refusing a level 3 Bug a
  ...
  if (breeding) {
    ...
    // a fainted Bug is a legal thing to have stored and therefore a legal
...
static void draw_wait(void) {
  ...
  if (link_trade_wants_consent()) {
    ...
    const SpeciesDef* give = species_get(s_tl.out_rec[BUGW_OFF_SPECIES]);
    const SpeciesDef* get  = species_get(s_tl.in_rec[BUGW_OFF_SPECIES]);
    line[0] = '\0';
