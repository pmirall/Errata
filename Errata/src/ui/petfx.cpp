...
// .txt file as the pixels now (ER_SPRITE_EYES in data/sprites_bugs.h) by one
...
//
// WHERE THE EYE BAND COMES FROM. data/sprites_bugs.h's ER_SPRITE_EYES, read
...
static uint32_t s_identity    = 0;     // bug_identity(), for auto-reseed
...
// petfx_reset(), which ui.cpp calls on an IDENTITY change - and a Bug that
// gets corrupted is the same Bug, so nothing would ever re-derive without
...
// Two custom Bugs share a set id, so without this the second one to be drawn
...
// The identity is DERIVED BY pet_view.cpp now (bug_identity: the Bug's
...
// the authored BUGSPR_SLEEP body for BOTH frames; deciding per frame would flip
...
// drawing, or nullptr for every Bug with an atlas row. THIS MODULE STILL DOES
...
// purposes, which is what s_cache_cst is for: two different creator Bugs
...
static void pf_cache_sync(uint8_t set_id, uint8_t mirrored, uint8_t sleeping,
  ...
  for (uint8_t pass = 0; pass < 2u; ++pass) {
    ...
    // pass exists to reach the authored BUGSPR_SLEEP blob, and a custom drawing
...
void petfx_service(const PetView& p, uint32_t now_ms) {
  ...
  // and teleports the body to its anchor, and a Bug catching a virus must
...
void petfx_draw_body(const PetView& p, uint8_t pose, uint8_t frame, int16_t dy,
  ...
  // writer, and an EGG therefore kept whichever creator body the LAST Bug
  ...
  if (p.stage == STAGE_EGG) {
    ...
    set_id = (uint8_t)(((uint32_t)p.age_s + 60u >= AGE_EGG_S)
                       ? (uint8_t)BUGSPR_EGG_CRACK : (uint8_t)BUGSPR_EGG_IDLE);
  } else {
    ...
    // (or, for a Bug with no row, the genome nibble). Since P9-C3 there is
...
void petfx_pose_ink_x(uint8_t pose, int16_t* x0, int16_t* x1) {
  ...
  if (s_qry_ok && s_qry_custom == nullptr) {
    ...
    // - for POSE_SLEEP that is still the BUGSPR_SLEEP blob, not the composited
