...
//   * the BOX and the active Bug - because persistence/save_manager.cpp's
...
// the fiction rather than about the stack: a wild Bug you have just beaten is
...
//
// WHO IS PAID: THE ACTIVE BUG, which is not necessarily one that fought.
...
// the XP and the level-up hp rescale on the Bug that stayed home. That is
...
// needs a per-Bug ledger and belongs with P4-C5's real battles.
...
// cooldown table, a changed bag or a changed Bug on flash, and it polls
...
// It said "a changed Bug" and the body wrote gs_save_active(), which commits
// THE ACTIVE SLOT AND NOTHING ELSE. A capture is filed by box_new_bug() into
...
// A fresh sealed genome for a captured Bug. It draws through RNG_BREEDING
...
// The Bug the player is carrying, or NULL. A screen that only wants to point
...
// The undocumented god-mode entry hold, 0..100, painted by the BUG screen's
...
// one, then the SPECIES the active Bug is (the roster's Spanish name), then
// the deterministic dynasty name for a Bug with no species row. Always
...
// FIRST BOOT: replace the Bug app/app.cpp minted with one of the chosen
...
// independently refuses any Bug that has earned or been named anything. Two
// locks, because this is the one call in the tree that destroys the ACTIVE
// Bug - box_release() will not, by rule B4.
