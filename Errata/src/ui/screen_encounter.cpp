// =============================================================================
//  ERRATA - ui/screen_encounter.cpp
...
// reason ui/battle_renderer.cpp passes them: a wild Bug on this screen has no
...
//           failing, which is what a wild Bug IS in this fiction - something
...
{
  const BugInstance* p = ui_active_bug();
...
void encounter_enter(void)
{
  ...
  if (s_enc.outcome == (uint8_t)ENC_OUT_SPECIAL) {
    ...
    ui_explore_commit((uint8_t)BOX_SLOT_NONE);   // corruption is on the ACTIVE Bug
    s_applied = 1u;
  }
}
...
void encounter_input(Gesture g)
{
  ...
  if (g == (Gesture)GST_HOLD_L) {
    switch (s_cursor) {
      ...
      // wild Bug you have just beaten is not still standing here waiting to
...
void encounter_render(void)
{
  ...
  switch ((EncounterOutcome)s_enc.outcome) {
    case ENC_OUT_WILD: {
      ...
      //  and the two options underneath. "BUG SALVAJE" is gone as a line -
...
static void throw_once(void)
{
  ...
  // EMPTY. cap_attempt() files through box_new_bug() -> first_free(), and
