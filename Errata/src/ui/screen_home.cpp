...
//  schema maximum, ER_NICKNAME_CAP - 1) and a two-digit level must not be able
//  to collide, at any font, on any Bug.
// -----------------------------------------------------------------------------
static void draw_identity(const BugView& v) {
  char lv[12];
...
static void draw_xp_rule(const BugView& v) {
...
static void draw_meters(const BugView& v) {
...
//  snapshot, shows the Bug where the device shows it.
// -----------------------------------------------------------------------------
...
// different creator Bugs share - they have no atlas row of their own and both
...
// draw. 0 means fall back to the authored BUGSPR_SLEEP blob - a blank frame or a
...
// species 1's silhouette under a custom Bug the moment it fell asleep - the
...
static void draw_static_body(const BugView& v, uint8_t frame) {
