...
void box_enter(void) {
  s_mode = BOXM_LIST;
  s_slot = 0;
  // Open on the Bug the player is carrying: that is the row they came to
...
//  ROW TEXT
//
//  "1 CANTO" plus a right-aligned "Nv7", and a leading '*' on the Bug that
//  is out walking. A slot with no nickname shows its SPECIES: inventing a name
//  here would disagree with the one HOME shows for the same Bug.
//
...
//  word is the fallback for a Bug with no species row, which is the same
//  ladder ui_pet_name() walks.
// -----------------------------------------------------------------------------
static const char* slot_species_name(const BugInstance& p) {
...
// -----------------------------------------------------------------------------
//  THE STORED BUG'S CARD
//
//  A stored Bug is NOT simulated: it has no smoothed bars, no mood face and
//  no bond score, because none of those are stored per instance. The BUG
...
// -----------------------------------------------------------------------------
static const uint16_t kCareLabel[ER_CARE_COUNT] = {
...
static uint8_t care_pct(int32_t milli) {
  if (milli <= 0) return 0;
  if (milli >= ER_CARE_MILLI_MAX) return 100;
  return (uint8_t)(milli / (ER_CARE_MILLI_MAX / 100L));
}

static void draw_card(void) {
  const BugInstance* p = box_peek(s_slot);
  ...
  // stored Bug is that these RECOVER, so they are what the card is for.
...
static void choose_action(void) {
  switch (s_cur) {
    ...
    case BOXA_SWAP:
      ...
      break;
    case BOXA_RELEASE:
      // B4: releasing the Bug you are carrying is refused outright, and the
    ...
    case BOXA_BREED:
      ...
      // is where the player is already looking at the Bug they mean, so this
