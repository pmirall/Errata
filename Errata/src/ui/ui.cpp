...
// game/bug.cpp, which a host binary can link; this file is now the LOOKUP
// and nothing else. What that bought is written in bug.h: the algorithm had
...
// drawUTF8() a broken lead byte. bug_name_join() truncates on a CHARACTER
...
void ui_name_for(uint32_t lineage_id, uint8_t generation, char* out, size_t cap) {
  if (!out || cap == 0) return;
  uint8_t syl[2];
  bug_name_syllables(lineage_id, generation, syl);
  const uint16_t room = (cap > 0xFFFFu) ? 0xFFFFu : (uint16_t)cap;
  (void)bug_name_join(S_SYL_A(syl[0]), S_SYL_B(syl[1]), out, room);
}
...
//   2. THE SPECIES the Bug is, from the roster's own Spanish name
...
//   3. the dynasty syllables, unchanged, for a Bug with no species row -
...
static void pet_name_stored(char* out, size_t cap) {
  ...
  if (!p) {
    ...
    const uint8_t slot = box_active();
    const BugInstance* pb = (slot == BOX_ACTIVE_NONE) ? nullptr : box_peek(slot);
...
// the Bug is doing, not how well it is being looked after.
...
// sleeping bug. The caller keeps whatever destination it had, and the toast
...
static bool home_body(uint8_t frame) {
  ...
  // mood score itself is on the BUG page where the rest of the numbers are.
...
static void dialog_commit(uint8_t which) {
  switch (which) {
    ...
    // the Bug AND the nvs2 checkpoint - before it returns, so by the time
    ...
    // it is the one action in the game that destroys a Bug (spec section 9,
    // invariant B4).
    ...
    case CFM_BOX_REL2: {
      if (box_release(s_box_slot, true)) {
        ...
        // SWAP / RELEASE offered for a Bug that no longer exists.
        box_screen_to_list();
    ...
    case CFM_WIPE2: {
      ...
      // BugInstance the simulation is still bound to and MINTS NO SLOT - it
      // is not a Box constructor (game/box.h calls box_new_bug() "THE TREE'S
...
static const BugView* ui_fill_view(void);
...
void ui_box_activate(uint8_t slot) {
  ...
  BugInstance* next = box_slot(slot);
  if (!next) { ui_toast(STR_BOX_EMPTY); return; }
  // FLUSH THE OUTGOING BUG FIRST. gs_save_active() only ever writes the slot
  // box.active_slot names, so once the index moves the Bug being put away
...
// player: box_reroll_starter() refuses any Bug that has earned or been named
...
bool ui_set_starter(uint8_t species_id) {
  ...
  if (!p) return false;
  // Same tail as ui_box_activate(): the simulation is rebound to the Bug
...
void ui_battle_result(uint8_t entry, uint8_t won) {
  ...
  // continuously, belonging to the device rather than to the Bug.
  //
...
//  save_box_header(), save_trade_journal(), save_bug_now() and
//  save_checkpoint_all() all landed real bytes, and a whole trade committed ten
//  flash writes - the outgoing Bug gone from flash, the incoming one on it.
//
...
static bool ui_tr_store_slot(void* ctx, uint8_t slot)
{
  ...
  // save_bug_now(), NOT save_bug(..., true). B1 and B2 write the SAME key
  // microseconds apart whenever box_add() reuses the slot B1 released, and
  // save_bug() DEFERS a second write inside SAVE_MIN_GAP_MS and returns
...
// networking/trade_link.cpp, so what is left is what a legal Bug may do.
...
BugInstance* ui_active_bug(void)
{
  const uint8_t slot = box_active();
