...
static void draw_intro(void) {
  ...
  if (ph == (uint8_t)SU_IN_TYPE || ph == (uint8_t)SU_IN_BUILD) {
    ...
    // listing is: `bug.c` is not copy, it is the name of the thing being
...
static void pick_commit(void) {
  ...
  // the player will remember. SKIP (A+B) keeps the default Bug and gets no
...
void setup_pick_input(Gesture g) {
  ...
  switch (g) {
    ...
    case GST_BOTH:
      // SKIP keeps the Bug app/app.cpp already minted, which is
