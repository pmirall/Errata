...
// CORRUPTED at once, and jumping a Bug eight levels were the same one word.
//
...
static void bag_use(void) {
  ...
  ItemEffect eff;
  const uint8_t r = inv_use(ui_inventory(), id, ui_active_bug(), now_epoch, cal, eff);
  switch ((ItemUse)r) {
  ...
  // The item acts on the ACTIVE Bug, which gs_save_active() already covers.
