...
//  module carried a PetSave <-> BugInstance map and wrote the live pet a
//  second time under the companion key "lgpet". game/sim.cpp now runs on
//  BugInstance directly (sim_bind), so the maps, the companion blob and the
...
// =============================================================================
#ifndef ER_GAME_STATE_H
#define ER_GAME_STATE_H
...
// second-guesses it.
//
// On LOAD_FRESH there is no Bug in the Box yet and gs_have_bug() is
...
// True when the loaded Box holds at least one Bug.
bool       gs_have_bug(void);
...
// -----------------------------------------------------------------------------
// Writes. gs_save_active() commits the ACTIVE Bug through save_bug() and,
