// =============================================================================
//  ERRATA - persistence/save_schema.h
...
// =============================================================================
#ifndef ER_SAVE_SCHEMA_H
#define ER_SAVE_SCHEMA_H

#include <stdint.h>
#include <stddef.h>

#include "../core/nt_types.h"     // Genome (copied whole into a Bug)
...
// 0. Shared constants
// -----------------------------------------------------------------------------
#define BOX_SLOTS               10      // spec section 9: ten Bugs, no more
#define ER_CARE_COUNT            5      // HUNGER HAPPINESS HEALTH CLEANLINESS ENERGY
#define ER_MOVE_COUNT            4
#define ER_NICKNAME_CAP         13      // NAME_MAX_LEN 12 + NUL
...
#define ER_NAME_DRAW_CAP        (2 * (ER_NICKNAME_CAP - 1) + 1)   // 25
#define ER_LEVEL_MAX            30      // spec section 11: levels 1..30
...
static_assert((int)CARE_COUNT == ER_CARE_COUNT, "CareId and care[] are out of sync");
...
// -----------------------------------------------------------------------------
// 1. BugInstance - 128 B, NVS keys "pb<slot><copy>" (pair), plan 1.5.1
//
...
// -----------------------------------------------------------------------------
// THE BYTES DID NOT MOVE WHEN THE NAME DID, and nobody may "fix" that.
// 0x4250 is 'P','B' - Pebblebol, the product's name until the rename - and it
// is written on every record already on somebody's flash and on every wire
// frame a peer that has not updated will send. Change it and every existing
// save is rejected as foreign and every un-updated peer becomes unreachable.
// The name is a name; the value is a contract with devices in the field.
#define BUG_MAGIC            0x4250u   // bytes 'P','B': the OLD name, on purpose
#define BUG_LAYOUT_VER       1         // the schema version lives in BoxHeader
#define BUG_CRC_BYTES        126
...
#define ER_CUSTOM_SPRITE_NONE   0xFFu
#define ER_CARE_MILLI_MAX       100000L   // care[] is milli-points, 0..100000

NT_PACK_PUSH
struct NT_PACKED BugInstance {
  uint16_t magic;                        //   0  BUG_MAGIC
  uint8_t  layout_ver;                   //   2  BUG_LAYOUT_VER
  ...
  int32_t  care[ER_CARE_COUNT];          //  28  milli-points, indexed by CareId
  int16_t  care_rem[ER_CARE_COUNT];      //  48  exact integrator remainders
  uint16_t hp_cur;                       //  58  current battle HP (max is derived)
  uint8_t  status;                       //  60  PBS_*
  uint8_t  flags;                        //  61  PBF_*
  uint8_t  moves[ER_MOVE_COUNT];         //  62  AttackId, 0 = empty
  uint8_t  origin;                       //  66  BugOrigin
  ...
  uint8_t  trades;                       //  75
  uint32_t lifetime_active_s;            //  76  seconds spent as the active Bug
  Genome   genome;                       //  80  the existing 16 B genome, whole
  char     nickname[ER_NICKNAME_CAP];    //  96  NUL-terminated, may be empty
  uint8_t  custom_sprite;                // 109  ER_CUSTOM_SPRITE_NONE or a cs slot
  ...
  // BUGW_STATUS_MASK already refuses PBS_CORRUPTED outright because the bit is
...
static_assert(sizeof(BugInstance) == 128, "BugInstance layout drifted");
static_assert(offsetof(BugInstance, species_id) ==   3, "BugInstance.species_id moved");
static_assert(offsetof(BugInstance, care)       ==  28, "BugInstance.care moved");
static_assert(offsetof(BugInstance, care_rem)   ==  48, "BugInstance.care_rem moved");
static_assert(offsetof(BugInstance, moves)      ==  62, "BugInstance.moves moved");
static_assert(offsetof(BugInstance, genome)     ==  80, "BugInstance.genome moved");
static_assert(offsetof(BugInstance, nickname)   ==  96, "BugInstance.nickname moved");
static_assert(offsetof(BugInstance, seq)        == 110, "BugInstance.seq moved");
static_assert(offsetof(BugInstance, corrupt_until_epoch) == 114,
              "BugInstance.corrupt_until_epoch moved - it was carved out of "
              "reserved[12] and every byte after it must stay where it was");
static_assert(offsetof(BugInstance, reserved)   == 118, "BugInstance.reserved moved");
static_assert(offsetof(BugInstance, crc16)      == 126, "BugInstance.crc16 moved");
static_assert(BUG_CRC_BYTES == sizeof(BugInstance) - 2, "BugInstance CRC span drifted");

// -----------------------------------------------------------------------------
// 2. BoxHeader - 32 B, keys "box0"/"box1" (pair), plan 1.5.3
//    The Box index: which slots hold a Bug, which one is active, and the
...
struct BoxHeader {
  ...
  uint16_t slot_mask;          //  4  bit s set = slot s holds a Bug
  uint16_t content_version;    //  6  CONTENT_VERSION the save was made against
  uint32_t next_id_counter;    //  8  monotonic; feeds the next Bug id
...
//    BugInstance.corrupt_until_epoch was carved out of reserved[12] at
...
//    Bug itself is never destroyed by a bad sprite.
// -----------------------------------------------------------------------------
...
struct CustomSpeciesRec {
  ...
  uint16_t budget_used;                               //   4  spec section 36:
                                                      ...
                                                      //      own Bug prices it at 0
  uint8_t  type;                                      //   6  BugType
  ...
  uint8_t  moves[ER_MOVE_COUNT];                      //  12  learnset
...
struct PendingTrade {
  ...
  uint32_t out_id;                       //  4  the Bug id we are sending
  uint32_t peer_id;                      //  8  device_id of the other side
  uint8_t  in_wire[TR_WIRE_BYTES];       // 12  the incoming Bug, wire form
...
// -----------------------------------------------------------------------------
struct GameState {
  BugInstance bugs[BOX_SLOTS];     //    0  10 x 128
  BoxHeader      box;                    // 1280
  ConfigV2       cfg;                    // 1312
  Inventory      inv;                    // 1568
...
// "pb" for the same reason BUG_MAGIC is 0x4250: it is the key every stored
// creature is ALREADY filed under. Renaming it to "bug" would not migrate a
// save, it would hide one.
#define KEY_BUG_PREFIX       "pb"      // pb<slot><copy>, the pre-rename key
...
#define KEY_CK_BUG_PREFIX    "ck_pb"   // ck_pb0..ck_pb9
...
// -----------------------------------------------------------------------------
void bug_seal(BugInstance& p);
bool bug_blob_ok(const BugInstance& p);          // magic + layout_ver + CRC
...
// True when the slot holds no Bug at all (species_id and id both zero).
...
inline bool bug_is_empty(const BugInstance& p) {
