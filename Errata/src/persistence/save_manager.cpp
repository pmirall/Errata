// =============================================================================
//  ERRATA - persistence/save_manager.cpp
...
static_assert(sizeof(CooldownTable) >= sizeof(BugInstance),"blob buffer too small");
...
const char* key_ck_bug(uint8_t slot, char* out) {
  ...
  key_copy(out, KEY_CK_BUG_PREFIX);
  const size_t n = strlen(out);
  out[n]     = (char)('0' + slot);
  out[n + 1] = '\0';
  return out;
...
// THE BUG IS ON ITS OWN VERSION AXIS and must not be folded into the
// schema's. BugInstance.layout_ver is BUG_LAYOUT_VER, which has not
...
// save as v1 on the strength of a Bug that is exactly what it should be.
static const BlobOps OPS_BUG = { BUG_MAGIC, BUG_LAYOUT_VER, BUG_LAYOUT_VER,
                                    sizeof(BugInstance),
                                    offsetof(BugInstance, layout_ver),
                                    offsetof(BugInstance, seq) };
...
void bug_seal(BugInstance& p)          { blob_seal(&p, OPS_BUG); }
bool bug_blob_ok(const BugInstance& p) { return blob_ok(&p, OPS_BUG); }
...
static bool     s_landed        = false;   // the last save_bug() reached flash
...
//  PART 4 - writing
// =============================================================================
bool save_bug_landed(void) { return s_landed; }
...
bool save_bug(uint8_t slot, const BugInstance& p, bool force) {
  ...
  if (!pair_write(KV_MAIN, prefix, OPS_BUG, &blob)) return false;
...
// cannot use save_bug() and why force=true was not widened to mean this.
bool save_bug_now(uint8_t slot, const BugInstance& p) {
  ...
  // The throttle's bookkeeping is updated exactly as save_bug() updates it.
  // Clearing the pending bit COSTS ONE FEWER FLASH WRITE and nothing more: a
  // deferred flush that did fire would write s_state->bugs[slot], which is
...
void save_service(void) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    s_pending_mask &= (uint16_t)~(1u << slot);
    save_bug(slot, s_state->bugs[slot], true);
  }
}
...
bool save_checkpoint_all(void) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    BugInstance p = s_state->bugs[slot];
    ok = single_write(KV_CKPT, key, OPS_BUG, &p) && ok;
  }
...
// than committed back at a version it never carried. The per-Bug records are
// deliberately not folded in: they carry BUG_LAYOUT_VER, a different axis.
static bool checkpoint_load(GameState& gs, uint8_t& found_ver) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    key_ck_bug(slot, key);
    BugInstance p;
    if (!single_load(KV_CKPT, key, OPS_BUG, &p)) continue;
    if (bug_is_empty(p)) continue;
    gs.bugs[slot] = p;
...
static void state_defaults(GameState& gs) {
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) bug_clear(gs.bugs[slot]);
  box_defaults(gs.box);
...
static bool scan_foreign(void) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    for (uint8_t copy = 0; copy < 2; ++copy) {
      char key[KV_KEY_CAP];
      key_bug(slot, copy, key);
      if (kv_get(KV_MAIN, key, tmp, OPS_BUG.size) != (int)OPS_BUG.size) continue;
      if (!blob_intact(tmp, OPS_BUG)) continue;
      if (tmp[OPS_BUG.ver_off] > OPS_BUG.version) return true;
    }
  }
  return false;
}
...
static bool commit_all(GameState& gs) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    BugInstance p = gs.bugs[slot];
    char prefix[KV_KEY_CAP];
    prefix[0] = 'p'; prefix[1] = 'b'; prefix[2] = (char)('0' + slot); prefix[3] = '\0';
    ok = pair_write(KV_MAIN, prefix, OPS_BUG, &p) && ok;
  }
...
//  game/validate.h takes its Bug CONST and this file could not repair even
//  if it wanted to. A quarantined Bug is loaded, drawn and reported with its
...
//  restore, because those two paths build Bugs too. On LOAD_CORRUPT and
...
static void quarantine_scan(const GameState& gs) {
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    if (bug_is_empty(gs.bugs[slot])) continue;
    const VReject r = validate_bug(gs.bugs[slot]);
    if (r != VR_OK) {
...
//  of the function. A creator Bug carries species_id 200..209, which
//  species_get() resolves through game/species_custom.cpp - so a scan that ran
//  first would answer VR_UNKNOWN_SPECIES for every custom Bug in the Box and
...
//  consequence is stated rather than hidden: the Bug that pointed at that
...
//  and a bad CRC costs a sprite rather than a Bug" - is about the CRC half;
...
static void custom_species_install_all(void) {
  ...
  for (uint8_t slot = 0; slot < (uint8_t)CUSTOM_SPECIES_SLOTS; ++slot) {
    ...
    // creature the owner made in the creator portal and quarantine the Bugs
...
static LoadResult load_all_inner(GameState& gs) {
  ...
  // blobs that carry SAVE_SCHEMA_VERSION feed it (see OPS_BUG's note).
  uint8_t found = (uint8_t)SAVE_SCHEMA_VERSION;
  ...
  for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
    ...
    BugInstance p;
    PairStat pst;
    pair_load(KV_MAIN, prefix, OPS_BUG, &p, pst);
    if (pst.good > 0 && !bug_is_empty(p)) {
      gs.bugs[slot] = p;
      truth_mask |= (uint16_t)(1u << slot);
      if (pst.bad > 0) repaired++;
    } else if (pst.bad > 0) {
      // Both copies of an occupied slot are unreadable. Losing the ACTIVE
      // Bug is what SAVE ERROR exists for; losing a stored one is a repair.
  ...
  if (repaired || mask_healed || active_healed) {
    ...
    for (uint8_t slot = 0; slot < BOX_SLOTS; ++slot) {
      if (truth_mask & (1u << slot)) {
        ...
        char prefix[KV_KEY_CAP];
        prefix[0] = 'p'; prefix[1] = 'b'; prefix[2] = (char)('0' + slot); prefix[3] = '\0';
        pair_write(KV_MAIN, prefix, OPS_BUG, &p);
      }
    }
