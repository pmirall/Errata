...
//  which slot is active, which slot is free and what a brand new Bug is.
//
...
// (plan 1.5.4). A slot that holds a Bug is one that is not empty, and the
...
static bool slot_filled(uint8_t slot)
{
  ...
  return !bug_is_empty(s_gs->bugs[slot]);
}
...
static void mask_sync(void)
{
  ...
  for (uint8_t i = 0; i < (uint8_t)BOX_SLOTS; ++i) {
    if (!bug_is_empty(s_gs->bugs[i])) m |= (uint16_t)(1u << i);
  }
...
const BugInstance* box_peek(uint8_t slot)
{
  return slot_filled(slot) ? &s_gs->bugs[slot] : nullptr;
}

BugInstance* box_slot(uint8_t slot)
{
  return slot_filled(slot) ? &s_gs->bugs[slot] : nullptr;
}
...
bool box_id_in_use(uint32_t id)
{
  ...
  for (uint8_t i = 0; i < (uint8_t)BOX_SLOTS; ++i) {
    if (slot_filled(i) && s_gs->bugs[i].id == id) return true;
  }
  return false;
}
...
uint8_t box_add(const BugInstance& p)
{
  if (!s_gs) return (uint8_t)BOX_SLOT_NONE;
  if (p.species_id == 0u) return (uint8_t)BOX_SLOT_NONE;   // not a Bug
  const uint8_t slot = first_free();
  if (slot == (uint8_t)BOX_SLOT_NONE) return slot;         // B1: ten and no more

  const uint32_t wanted = p.id;
  BugInstance& dst = s_gs->bugs[slot];
  dst = p;
  dst.magic      = (uint16_t)BUG_MAGIC;
  dst.layout_ver = (uint8_t)BUG_LAYOUT_VER;
  dst.seq        = 0;
...
// THE MINT, lifted out of box_new_bug() at P10-C4 so box_reroll_starter()
// below cannot become a second copy of it. Nothing about it changed.
static void mint(BugInstance& p, const SpeciesDef& sp, uint8_t species_id,
                 ...
                 uint32_t creation_seed, uint32_t now_epoch)
{
  ...
  p.level         = level;
  // THE STAGE BITS. box_new_bug() never wrote evo_state before P4-C5, so
  // every Bug it minted at a stage-1 or stage-2 species carried stage 0 - a
  ...
  // have refused a legitimately captured mid-stage Bug. THE FIX IS AT THE
  // WRITER, never a repair inside the validator: reverting this line turns
  // tests/test_validate.cpp's `a_constructed_bug_validates` red.
  // EVO_STATE_PENDING is deliberately NOT set here: game/xp.cpp raises it on a
  // level-up and validate_bug()'s rule is one-directional, so a wild capture
                       ...
                       uint32_t now_epoch)
{
  ...
  if (level > (uint8_t)ER_LEVEL_MAX) level = (uint8_t)ER_LEVEL_MAX;
...
//  release is a destruction and the active Bug is the one the player is
//  holding. But the starter choice has to replace exactly that Bug - the one
//  app/app.cpp minted at boot before the player had been asked anything - so it
//  needs its own door, and a door into "destroy the active Bug" needs a lock
//  that is about the BUG and not about when the call happens.
//
...
//  this refuses any Bug that has DONE anything: it must be an ORIGIN_STARTER
...
//  player did, and a Bug that has done none of them is worth exactly what
...
bool box_reroll_starter(uint8_t slot, uint8_t species_id, uint32_t now_epoch)
{
  ...
  const BugInstance& p = s_gs->bugs[slot];
  ...
  mint(s_gs->bugs[slot], *sp, species_id, 1u, (uint8_t)ORIGIN_STARTER,
...
bool box_swap(uint8_t a, uint8_t b)
{
  ...
  BugInstance tmp = s_gs->bugs[a];
  s_gs->bugs[a]   = s_gs->bugs[b];
  s_gs->bugs[b]   = tmp;

  // The active Bug is an INDEX, not a copy (plan 1.5.3), so a swap has to
  // carry the index with the creature or the player would be holding a
  // different Bug than the one they moved.
...
bool box_release(uint8_t slot, bool confirmed)
{
  ...
  memset(&s_gs->bugs[slot], 0, sizeof s_gs->bugs[slot]);
  s_gs->bugs[slot].magic      = (uint16_t)BUG_MAGIC;
  s_gs->bugs[slot].layout_ver = (uint8_t)BUG_LAYOUT_VER;
  mask_sync();
  return true;
}
...
void box_recover(uint8_t slot, uint32_t elapsed_s)
{
  ...
  for (uint8_t i = 0; i < (uint8_t)ER_CARE_COUNT; ++i) {
    ...
    if (v > (int64_t)ER_CARE_MILLI_MAX) {
      v   = (int64_t)ER_CARE_MILLI_MAX;           // B5: caps at 100 %
  ...
  // NOT touched: a Bug left in the Box comes back healthy, not different.
}
...
uint8_t box_recover_all(uint32_t now_epoch)
{
  ...
  for (uint8_t i = 0; i < (uint8_t)BOX_SLOTS; ++i) {
    ...
    const uint32_t last = s_gs->bugs[i].last_updated_epoch;
