...
const EvolutionRule* evolution_rule_for(uint8_t species_id)
{
  ...
  // at someone else's rule can never take a Bug out of its own family.
...
bool evolution_apply(BugInstance& p, const EvoContext& ctx)
{
  ...
  // rescale as a level-up, from the same pair of functions, so a full Bug
