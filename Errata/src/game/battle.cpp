...
#include "bug.h"     // bug_derive_stats(): the ONE stat derivation
...
// Bug that was just killed answer the blow that killed it.
...
{
  if (slot >= (uint8_t)ER_MOVE_COUNT) return false;
...
BattleReject battle_action_legal_now(const BattleState& st, uint8_t side, BattleAction act)
{
  ...
  if (act.kind == (uint8_t)BACT_ATTACK && act.index >= (uint8_t)ER_MOVE_COUNT) return BR_BAD_INDEX;
  ...
  if (act.kind == (uint8_t)BACT_ATTACK) {
    // ONE guard, not two. 0 is BugInstance's empty move slot and
...
// ONLY places a BugInstance.moves[] is ever written - and game/evolution.cpp
...
// a per-species rule would refuse every evolved Bug in the box.
...
// stats, so battle_init() derives once. game/bug.cpp owns the derivation and
// game/xp.h owns hp_max: this file holds no stat formula at all.
static BattleReject setup_member_ok(const BugInstance& p,
                                    const SpeciesDef*& sp_out, BugStats& stats_out)
{
  sp_out = nullptr;
  ...
  if (p.level < 1u || p.level > (uint8_t)ER_LEVEL_MAX) return BR_BAD_LEVEL;
  for (uint8_t m = 0; m < (uint8_t)ER_MOVE_COUNT; ++m)
    if (attack_get(p.moves[m]) == nullptr) return BR_ILLEGAL_MOVESET;
  // Four real attacks is not four attacks THIS Bug could know.
...
BattleReject battle_init(BattleState& st, const BattleSetup& setup)
{
  ...
  for (uint8_t s = 0; s < 2u; ++s) {
    for (uint8_t i = 0; i < setup.count[s]; ++i) {
      ...
      // corrupt_left since P4-C2 and nothing ever set it from the Bug, so a
      ...
      for (uint8_t m = 0; m < (uint8_t)ER_MOVE_COUNT; ++m) c.moves[m] = p.moves[m];
...
int8_t battle_s2_priority(const BattleState& st, uint8_t side)
{
  ...
  if (me.pending_index >= (uint8_t)ER_MOVE_COUNT) return 0;
                               ...
                               int8_t* spent)
{
  ...
  // roll add up to 2 back on top, and a protected Bug could take more than
...
//  (sim_engine.py calls end_of_round(A); end_of_round(B)). A benched Bug's
...
//  in the status tick that deals damage - so without this line a Bug killed
...
//  battle_alive_count() and step 9 already count a 0-HP unflagged Bug as
...
BattleOutcome battle_s9_check_victory(const BattleState& st)
{
  ...
  if (st.round > (uint16_t)BATTLE_MAX_ROUNDS) {
    ...
    // Bugs. An exact tie is an honest DRAW - there is no coin flip in this
    ...
    // hp_max 58 at level 30, so a three-Bug side sums to at most 174 and the
