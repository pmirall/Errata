// =============================================================================
//  ERRATA - game/bug.cpp
//  Derived stats. See bug.h for the formulas, the integer discipline and
...
uint8_t bug_genome_var(uint8_t gene_value)
{
  ...
  const uint8_t out = (uint8_t)((v * (uint16_t)(BUG_GENOME_VAR_MAX + 1)) / 16u);
  return (out > BUG_GENOME_VAR_MAX) ? (uint8_t)BUG_GENOME_VAR_MAX : out;
}

void bug_genome_vars(const Genome& g, uint8_t out[3])
{
  if (!out) return;
  out[0] = bug_genome_var(gene_temperament(g));   // atk
  out[1] = bug_genome_var(gene_hardiness(g));     // def
  out[2] = bug_genome_var(gene_metabolism(g));    // spd
}
...
bool bug_stats_of(const BugInstance& p, BugStats& out)
{
  ...
  bug_derive_stats(*sp, p.level, p.genome, out);
  return true;
}

// =============================================================================
//  THE DYNASTY NAME. See bug.h for what moved out of ui/ui.cpp and what
...
void bug_name_syllables(uint32_t lineage_id, uint8_t generation, uint8_t out[2])
{
  ...
  out[0] = (uint8_t)(h % (uint32_t)ER_NAME_SYLLABLES);
  out[1] = (uint8_t)((h / (uint32_t)ER_NAME_SYLLABLES) % (uint32_t)ER_NAME_SYLLABLES);
}
...
uint8_t bug_name_join(const char* a, const char* b, char* out, uint16_t cap)
{
  ...
  // not the beginning of their Bug's name. A prefix is a legible truncation;
