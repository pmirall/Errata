// =============================================================================
//  ERRATA - game/box_sim.h
//  THE TWO OPERATIONS THAT TOUCH BOTH THE BOX AND THE RUNNING SIMULATION.
//
//  game/box.cpp stores the ten Bugs BY VALUE inside the single GameState and
...
//  BugInstance in memory leaves sim's pointer stale unless somebody repairs
...
// the creature, so when the active slot is one of the two the active Bug
// ends up at the OTHER index while sim is still bound to the old address - i.e.
// the player keeps playing the Bug that moved into the slot they left. This
