...
//  that resolution, and it is the reason a Bug the creator made is an
//  ORDINARY Bug everywhere else in the tree.
//
//  ==========================================================================
...
//
//    * game/box.cpp's box_new_bug() - the tree's ONE Bug constructor -
//      returns BOX_SLOT_NONE when species_get() answers nullptr, so a creator
//      Bug could not be built at all.
...
//      runs validate_bug() over every occupied slot and VR_UNKNOWN_SPECIES
...
//                     validate_bug()'s VR_BAD_EVO_STAGE rule and the budget
...
//                     Bug, not a species the world can hand out again.
//
...
//  resolve. So a custom Bug is local until a later phase sends the record
...
// =============================================================================
#ifndef ER_GAME_SPECIES_CUSTOM_H
#define ER_GAME_SPECIES_CUSTOM_H
...
// Bug would keep resolving to a species the device has forgotten.
void csp_reset(void);
...
// its Bug is quarantined by name at the next scan.
...
// species of a Bug already in the Box.
...
//  body in data/sprites_bugs.h, which is why wiring them into the renderer
...
//  Bug came out wearing somebody else's face.
//
...
static_assert(CREATOR_SPECIES_ID_MAX <= 255u,
              "a custom species id must fit BugInstance.species_id");

#endif  // ER_GAME_SPECIES_CUSTOM_H
