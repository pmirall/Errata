...
//  the ACTIVE Bug - the one game/sim.cpp is bound to and the only one that
...
//        field - a Bug left in the Box comes back healthy, not different
//
...
const BugInstance* box_peek(uint8_t slot);     // nullptr when empty/out of range
BugInstance*       box_slot(uint8_t slot);     // mutable, same rules
...
// non-zero and unused, and minted otherwise, so a captured or traded Bug
...
// Builds a brand new Bug of `species_id` at `level` and files it (B2).
...
// FIRST BOOT ONLY IN PRACTICE, BUT THE RULE IS ABOUT THE BUG. Replace the
// Bug in `slot` with a freshly minted one of `species_id`, keeping the slot,
...
// -----------------------------------------------------------------------------
// Recovery (B5). Stored Bugs heal, they do not live.
// -----------------------------------------------------------------------------
...
// The next Bug id: hash32(device_id, next_id_counter), never 0, never one
