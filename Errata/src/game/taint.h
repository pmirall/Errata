...
//  this header reads are the GENOME BIT and the BugInstance FLAG, both of
...
//  WHERE IT IS NOT
// -----------------------------------------------------------------------------
//   * NOT in validate_bug(). game/validate.h says twice that there is no
//     policy parameter, and PBF_GOD_TAINTED is INSIDE VLD_FLAGS_MASK on
//     purpose: a tainted Bug is a perfectly legal OBJECT. A rule here would
//     quarantine every tainted Bug on the save path, i.e. brick the device
...
//      onto the 48 B wire record at BUGW_OFF_GENOME. This is the DURABLE truth.
//   2. BugInstance.flags bit PBF_GOD_TAINTED. Set by game/sim.cpp as a
//      mirror of the genome bit when a Bug is bound, and INDEPENDENTLY by
//      persistence/migration.cpp from the v1 LV1_PF_GOD_TAINTED - a v1 save can
//      therefore carry the flag with the genome bit clear.
//
//  They are set by different writers on different paths, so bug_is_tainted()
//  reads BOTH. Reading only the genome misses every migrated v1 unit; reading
//  only the flag misses a Bug god mode touched before sim bound it.
//
// -----------------------------------------------------------------------------
...
// =============================================================================
#ifndef ER_GAME_TAINT_H
#define ER_GAME_TAINT_H

#include <stdint.h>

#include "../persistence/save_schema.h"   // BugInstance, PBF_GOD_TAINTED
#include "genome.h"                       // gene_tainted()

// True when EITHER marker is set. See the banner for why both are read.
inline bool bug_is_tainted(const BugInstance& p)
{
...
// game/trade.cpp and game/breeding.cpp. `local` is a Bug this device owns;
...
inline bool taint_gate_ok(const BugInstance& local, const BugInstance& incoming)
{
  return !bug_is_tainted(incoming) || bug_is_tainted(local);
}

#endif  // ER_GAME_TAINT_H
