...
{
  // Distinct UNITS, by id. Not by pointer and not by species: two Bugs of
  // one species are a perfectly good pair, and the same Bug handed in twice
  // is the thing spec section 9 calls a duplication.
  if (a.id == 0u || b.id == 0u || a.id == b.id) return BRD_SAME_UNIT;

  // THE SPECIES LOOKUP COMES BEFORE validate_bug() AND THAT ORDER IS THE
  ...
  if (validate_bug(a) != VR_OK || validate_bug(b) != VR_OK)
  ...
  // put one of its own Bugs into the same pot.
...
BreedReject breed_commit(const BreedPlan& plan, uint32_t now_epoch, uint8_t& slot_out)
{
  ...
  // THE TREE'S ONE CONSTRUCTOR. box_new_bug() owns evo_state, the learnset,
  ...
  const uint8_t slot = box_new_bug(plan.species_id, plan.level,
  ...
  // it has a Bug. A failure here is a bug in this file - never a peer
  // capability - so the slot goes back rather than being quarantined later.
  if (validate_bug(*p) != VR_OK) {
