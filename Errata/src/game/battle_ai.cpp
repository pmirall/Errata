...
uint32_t battle_ai_move_score(const BattleState& st, uint8_t side, uint8_t move_slot)
{
  ...
  if (side > 1u || move_slot >= (uint8_t)ER_MOVE_COUNT) return 0u;
...
bool battle_ai_wants_to_switch(const BattleState& st, uint8_t side)
{
  ...
  // Bug at exactly the threshold stands and fights, and
  ...
  // ...AND a benched Bug with a STRICTLY better type. One number, not two:
