...
//
//  NOTHING DERIVED IS EVER STORED. A Bug carries its species id, its level,
...
//  fresh Bug draws 0 / 1 / 2 with probability 2/9, 5/9, 2/9 - symmetric,
...
//  picks a Temperament class, so a fast-metabolism Bug both burns energy and
...
// =============================================================================
#ifndef ER_GAME_BUG_H
#define ER_GAME_BUG_H
...
// 0..15 gene value -> 0..BUG_GENOME_VAR_MAX. Public because tests and the
...
uint8_t bug_genome_var(uint8_t gene_value);

// The three variation terms of one genome, in stat order (atk, def, spd).
void bug_genome_vars(const Genome& g, uint8_t out[3]);

// THE derivation. `level` is clamped into 1..ER_LEVEL_MAX; a level of 0 (an
// un-initialised or migrated Bug) is treated as 1 rather than producing a
// creature with no stats.
void bug_derive_stats(const SpeciesDef& sp, uint8_t level, const Genome& g,
                         BugStats& out);

// The same derivation straight off a stored Bug. Returns false and leaves
...
bool bug_stats_of(const BugInstance& p, BugStats& out);

// =============================================================================
//  THE DYNASTY NAME (spec section 54, moved out of ui/ui.cpp by P9-C4)
//
//  The name of a Bug with no species row is a PURE FUNCTION of
...
//    * bug.h's own banner says this is a pure module - stdint, the species
...
//      layer banner says "no I/O"; bug_name_join() below is the six lines
...
//  fold happens exactly once, here, and ER_NAME_SYLLABLES is the number both
//  sides now agree on through core/strings_es.h's own static_asserts.
// =============================================================================
#define ER_NAME_SYLLABLES 12

// The two syllable indices of the dynasty name, each already folded into
// 0..ER_NAME_SYLLABLES-1. out must have 2 elements; a null out is a no-op.
//
...
// 0x2545F491 mix. A different answer here would silently rename every Bug on
// every device that ever ran an earlier build, and persistence/migration.cpp
// reasons about that name in prose.
void bug_name_syllables(uint32_t lineage_id, uint8_t generation, uint8_t out[2]);
...
// word that is not the beginning of their Bug's name.
//
// cap == 0 writes nothing at all (there is nowhere to put a terminator); a null
// out writes nothing. Either answers 0.
uint8_t bug_name_join(const char* a, const char* b, char* out, uint16_t cap);
