...
//
//  A trade is the only operation in this firmware that DESTROYS a Bug on one
...
//  outgoing Bug or the incoming one, never both and never neither, with the
...
//  Bug is destroyed while the peer's is duplicated, responder first and it is
...
//  in_wire at BUGW_OFF_ID and the 48 B record is resealed, so the journal names
//  the id the incoming Bug WILL HAVE LOCALLY and both halves of the
...
// =============================================================================
#ifndef ER_GAME_TRADE_H
#define ER_GAME_TRADE_H
...
//  verdict (game/taint.h and game/breeding.h say the same), and every Bug
...
enum TradeReject : uint8_t {
  ...
  TDR_ACTIVE,            // THE BUG YOU ARE HOLDING. See the note below.
  ...
  TDR_INVALID,           // validate_bug() refused our OWN outgoing Bug
  // THERE IS NO TDR_LAST_BUG AND THIS IS WHERE THAT IS WRITTEN DOWN. "The
  // last Bug on the device cannot be given away" is a real rule and it is
  ...
  // lowest occupied slot, so in a Box with exactly one Bug that Bug IS
  ...
  TDR_DUPLICATE_ID,      // the incoming Bug carries an id we already hold
  ...
  TDR_LOST,              // a COMMIT record whose in_wire will not decode, with
                         // the outgoing Bug already gone. Unreachable while
...
enum TradeResolution : uint8_t {
  ...
  TRS_LOST,              // a COMMIT record whose in_wire will not decode, with
                         // the outgoing Bug ALREADY GONE. One Bug has
...
// -----------------------------------------------------------------------------
struct TradeCodec {
  // The 48 B record -> a complete, VALIDATED BugInstance. VR_OK or nothing:
  // `out` is untouched on every reject.
  VReject (*decode)(const uint8_t rec[TR_WIRE_BYTES], BugInstance& out);
...
struct TradeStore {
  ...
  bool (*write_slot)(void* ctx, uint8_t slot);   // persists gs.bugs[slot]
...
//  would drag kv_store into every binary that touches a Bug.
//
...
//  Bug you are holding would mean this module moving box.active_slot and
...
//  player can act on ("guarda este Bug primero"). THE RESOLVER STILL HANDLES
...
//  validate_bug() has run on it; what is left is the POLICY.
// -----------------------------------------------------------------------------
