// =============================================================================
//  ERRATA - game/corruption.cpp
//  See corruption.h. PURE translation unit.
// =============================================================================
#include "corruption.h"

// A slot with no creature in it cannot be ill. bug_is_empty() is the Box's
...
// finding.
static inline bool usable(const BugInstance& p)
{
  return !bug_is_empty(p);
}
...
bool cor_apply(BugInstance& p, uint32_t now_epoch, uint8_t cal)
{
  ...
  // REFRESH, NEVER STACK: a second corruption on an already corrupted Bug
...
bool cor_expire(BugInstance& p, uint32_t now_epoch, uint8_t cal)
{
  ...
  // A corrupted Bug with NO deadline is what a save written by a firmware
