// =============================================================================
//  ERRATA - game/capture.h
//  CAPTURE (spec section 23, plan P5-C4).
//
//  The chance, the attempt, the flee, and the one call that mints the creature.
//
// -----------------------------------------------------------------------------
//  A CAPTURED BUG MUST PASS THE VALIDATOR - the carried-forward debt this
//  file discharges, and what it actually took
// -----------------------------------------------------------------------------
//  game/validate.h records that box_new_bug(), the tree's ONE constructor,
//  deliberately does NOT call validate_bug(): five shipped host tests hand
...
//  on it: driving box_new_bug() -> validate_bug() across the whole
...
//  The genome seal is the ONLY input that can make a constructed Bug
...
//      a Bug the wire path would later reject from ever being filed.
...
//  Deleting the validate_bug() call entirely and hardcoding VR_OK leaves
//  every host case green - it was tried - because with a sealed genome the
//  answer is VR_OK on all 1,080 rows and the tests assert the FILED BUG is
...
//  learnsets disagree with validate_bug()'s family walk, which is exactly
//  what the P9 roster change risks - so it stays, as an assertion with its
//  falsifier stated rather than implied.
//
//  AND THE BUG IS NOT DESTROYED ON THAT PATH, which is a decision rather
//  than an omission. "File, validate, undo" reads better and CANNOT BE WRITTEN
//  CORRECTLY here: box_new_bug() mints AND files in one call, mask_sync()
//  makes the first Bug in an empty Box the ACTIVE one (invariant B3), and
...
//  "the encounter must never delete the active Bug" and section 55's "never
...
// =============================================================================
#ifndef ER_GAME_CAPTURE_H
#define ER_GAME_CAPTURE_H
...
enum CaptureOutcome : uint8_t {
  ...
  CAP_INTERNAL,       // constructed and then failed validate_bug(): a bug in
