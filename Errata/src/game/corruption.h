...
//  BugInstance.corrupt_until_epoch is four of the twelve reserved bytes
...
// =============================================================================
#ifndef ER_GAME_CORRUPTION_H
#define ER_GAME_CORRUPTION_H

#include <stdint.h>

#include "../data/balance.h"               // CORRUPT_DURATION_S
#include "../persistence/save_schema.h"    // BugInstance, PBS_CORRUPTED, TimeCal

// -----------------------------------------------------------------------------
// cor_apply(p, now_epoch, cal) -> was the Bug corrupted?
//
...
//   corrupted Bug EXTENDS the deadline rather than adding a second status,
//   which is the same rule game/battle.cpp's DOT uses: one representation per
//   fact, refreshed rather than stacked.
// -----------------------------------------------------------------------------
bool cor_apply(BugInstance& p, uint32_t now_epoch, uint8_t cal);
...
//   Bug that is not corrupted, and a no-op while the clock is CAL_UNSET: an
...
// -----------------------------------------------------------------------------
bool cor_expire(BugInstance& p, uint32_t now_epoch, uint8_t cal);
...
//   cor_expire() with NO CALLER anywhere in Errata/src: cor_apply() ran from
...
//   BOX and not over the active slot alone: a benched Bug's deadline passes
...
//
//   Empty slots and Bugs that are not corrupted cost one branch each and are
...
// -----------------------------------------------------------------------------
uint8_t cor_service(BugInstance* slots, uint8_t n, uint32_t now_epoch, uint8_t cal);
...
bool cor_is_corrupted(const BugInstance& p);

// Seconds left, 0 when not corrupted or already due. Diagnostics and tests; the
// screens do not draw it yet.
uint32_t cor_left_s(const BugInstance& p, uint32_t now_epoch, uint8_t cal);

#endif  // ER_GAME_CORRUPTION_H
