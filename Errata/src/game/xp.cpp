...
bool xp_add(BugInstance& p, uint16_t amount, XpSource src, uint8_t* levels_gained)
{
  ...
  // against either, so the budget is left alone for the Bug that is real.
  ...
  // Bug at XP_LEVEL_MAX spent nothing - and app/app.cpp's activity reward
  // reads the meter either side of the award to learn what the score was worth,
  // so a level-30 Bug earned ZERO activity happiness as well as zero XP.
  ...
  // what a day of play can pay, and a Bug that cannot use its share has
  ...
  // untouched, so a maxed Bug no longer banks the day for a Box-mate.
  ...
  // EVOLVE_PENDING, on every award rather than only on a level-up: a Bug
