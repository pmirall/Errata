...
//    * BugInstance has no held-item field either, and putting one there
//      would make the buff persisted AND wire-visible, so validate_bug()
...
// =============================================================================
#ifndef ER_GAME_INVENTORY_H
#define ER_GAME_INVENTORY_H
...
#define CARE_MILLI_PER_PCT  ((int32_t)(ER_CARE_MILLI_MAX / 100L))
static_assert(CARE_MILLI_PER_PCT * 100L == (int32_t)ER_CARE_MILLI_MAX,
...
enum ItemUse : uint8_t {
  ...
  IU_NO_TARGET,       // the klass needs a Bug and none was given
...
//   klasses that need no Bug) and consumes ONE on success. Nothing here
