...
enum ScreenId : uint8_t {
  ...
  SCR_STATUS,        // BUG page A: vitals
  SCR_STATUS_B,      // BUG page B: genome. See the note below.
...
// a new state: it is the second page of the same BUG state. The pages are
...
//    BugInstance (persistence/save_schema.h) and the only surviving
...
//    are mirrored into BugInstance.status / .flags by the sim.
// -----------------------------------------------------------------------------
// SimView.flags bits
...
//    displacing the living pet, so it cannot live on the Bug. Nothing writes
