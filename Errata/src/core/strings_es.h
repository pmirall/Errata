...
enum StrId : uint16_t {
  ...
  // are how the device says which - the alternative is a Bug silently
  ...
  // the outgoing Bug had already left and the incoming record will not
  ...
  // once and jumping a Bug eight levels all read the same single word, on
...
inline constexpr const char* const ES[] = {
  ...
  /* STR_BOX_REL_Q1 */            "¿Liberar a este Bug?",
  ...
  , /* STR_BT_DRAW */             "Empate"
  , /* STR_BT_NO_TEAM */          "Elige al menos un Bug"
...
static_assert(STR_MENU_SETTINGS   - STR_MENU_BUG + 1 == MENU_ITEM_COUNT,    "menu labels");
