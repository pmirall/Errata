...
//    A Bug cannot die. Neglect makes it inconveniently unhappy and nothing
...
//    back to a sad Bug, never to a lost one.
//
...
// =============================================================================
#define ER_BALANCE_CARE_COUNT   5
...
inline constexpr int32_t CARE_DECAY_MPH[ER_BALANCE_CARE_COUNT] = {
...
// then simply stops: an inconveniently unhappy Bug, never a dead one.
#define CARE_ZERO_GRACE_S       7200UL       // 2 h
#define HEALTH_FLOOR_PCT        10

// Box recovery (spec section 9 / section 27). A stored Bug is not simulated:
...
// Bug left in the Box overnight comes back full and a week is far past it.
...
//    12 h of cleanliness, so 3-4 touches a day is a well-kept Bug.
//
...
//    hours who touches the Bug 3-4 times a day lands 3.3 care actions a day
//    and its satiety reaches ZERO every night, after which CARE_ZERO_GRACE_S
//    expires and health bleeds to HEALTH_FLOOR_PCT. THE BUG IS NEVER LOST -
...
//    so swapping the active Bug is worthless as a farming move. Hitting
...
//    BugInstance.xp is always "XP inside the current level" and never a
...
//      * every entry fits u16 - BugInstance.xp is u16 (plan 1.5.1);
...
//        BugInstance.level through level_of_stage(), so a Bug that earns
...
//      * A NORMAL PLAYER CANNOT KEEP THE BUG FED - see section 2's note.
//
...
// quantity the u16 is about - BugInstance.xp holds xp toward the NEXT level,
...
static_assert(xp_table_total() < 65535u,
              "the whole XP curve must fit u16 (BugInstance.xp is u16)");
...
// Bug spends AWAKE with the device on. Asleep time and time in the Box pay
...
// Bug is worthless as a farming move, and it refills continuously rather
...
// break turn order and a fast Bug pays 6-7 of its 16/22/28 stat points for
...
// with this when the BugInstance carries PBS_CORRUPTED, and BATTLE_MAX_ROUNDS
...
// already-corrupted Bug hit by Infectar reaches +1 and not +2, and the
...
// Bug (spec section 22). P5-C3 was the first code that could start this
...
// 2. THE TEAM SIZE. Spec section 14 says "up to 3 Bugs" in prose and names
...
// AI at all, it scripts both sides. The comparison is STRICT, so a Bug at
...
static_assert(BATTLE_TEAM_MAX >= 1, "a team needs a Bug");
...
// =============================================================================
#define ER_DAYLIGHT_MONTHS      12
#define ER_MINUTES_PER_DAY      1440u

// Day of year (0-based, non-leap) of the 15th of each month: the anchor each
// sample below is taken at.
inline constexpr uint16_t DAYLIGHT_ANCHOR_DOY[ER_DAYLIGHT_MONTHS] = {
   14,  45,  73, 104, 134, 165, 195, 226, 257, 287, 318, 348
};
// Minutes from local midnight.
inline constexpr uint16_t DAYLIGHT_SUNRISE_MIN[ER_DAYLIGHT_MONTHS] = {
  515, 490, 445, 450, 410, 405, 420, 450, 480, 510, 490, 510
};
inline constexpr uint16_t DAYLIGHT_SUNSET_MIN[ER_DAYLIGHT_MONTHS] = {
...
// punishing it, and is what stops a player re-rolling one wild Bug forever.
