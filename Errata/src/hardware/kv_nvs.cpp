...
static void legacy_import(void) {
  ...
  Preferences old;
  if (!old.begin(ER_NVS_LEGACY_NS, true)) {
  ...
  Preferences wipe;
  if (wipe.begin(ER_NVS_LEGACY_NS, false)) {
    (void)wipe.clear();
    wipe.end();
  }
  s_imported = true;
...
bool kv_begin(void) {
  ...
  s_open[KV_MAIN] = s_prefs[KV_MAIN].begin(ER_NVS_NAMESPACE, false);
  if (!s_open[KV_MAIN]) {
    s_prefs[KV_MAIN].end();
    s_open[KV_MAIN] = s_prefs[KV_MAIN].begin(ER_NVS_NAMESPACE, false);
  }
  if (!s_open[KV_MAIN]) {
    kv_fail(KV_E_OPEN_MAIN, "begin(" ER_NVS_NAMESPACE ")", 0);
  }

  // The checkpoint partition. Missing "nvs2" is a degraded unit, not a dead
  // one: it loses D6's recovery, nothing else.
  s_open[KV_CKPT] = s_prefs[KV_CKPT].begin(ER_NVS_NAMESPACE, false, ER_NVS_CKPT_PART);
  if (!s_open[KV_CKPT]) {
    kv_fail(KV_E_OPEN_CKPT, "begin(" ER_NVS_CKPT_PART ")", 0);
  }
