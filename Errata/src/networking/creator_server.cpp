// =============================================================================
//  ERRATA - networking/creator_server.cpp
...
//  firmware's. Everything here is a COUNT or a VERSION; no Bug is serialised,
...
//
//  POST /api/validate and POST /api/bug read the SAME document and apply the
...
//  and refused by /api/bug is the worst outcome this feature has.
//
...
//  because an accepted definition whose Bug could not be filed would leak a
...
//  change the species of a Bug already in the Box.
static void h_bug(void)
{
  ...
  // creator Bug over that is exactly the silent overwrite gs_set_readonly()
  // exists to stop.
  ...
  // Box. Filing the first Bug moves box_active() from BOX_ACTIVE_NONE to a
  // real slot (invariant B3), and game/sim.cpp is bound to the ACTIVE Bug by
  ...
  const uint8_t slot = box_new_bug(species_id, 1u, (uint8_t)ORIGIN_CREATOR,
                                      ...
                                      gt_now());
  BugInstance* p = (slot == (uint8_t)BOX_SLOT_NONE) ? nullptr : box_slot(slot);
  if (p == nullptr) {
    csp_forget(cs_slot);
    send_err(500, "box");
    cs_body_done();
  ...
  // The two fields box_new_bug() cannot know, set at the CONSTRUCTOR's own
  // moment rather than mended afterwards: the constructor takes a species id and
  // a level, and a cs slot is neither. validate_bug() below is what checks
  ...
  static_assert(sizeof(((BugInstance*)nullptr)->nickname) == CS_NAME_CAP,
  ...
  const VReject vr = validate_bug(*p);
  if (vr != VR_OK) {
    bug_clear(*p);
  ...
  // Bug on flash whose cs record is not there yet comes back quarantined
  // (VR_UNKNOWN_SPECIES), while a cs record whose Bug is not there yet is
  ...
  // its RAM state for every other failure (csp_install, box_new_bug,
  // validate_bug, save_custom_species, all above). When it failed the client
  ...
  // got 200 {"ok":1,...} and the Bug was gone after a power cycle.
  //
  ...
  // csp_forget() the slot back. A creator slot spent with no Bug pointing at
...
void cs_register(WebServer& srv, WebServer::THandlerFunction notfound)
{
  ...
  srv.on("/api/bug",   HTTP_POST, h_bug,   cs_body_hook);
