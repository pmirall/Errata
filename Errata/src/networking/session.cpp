...
uint16_t session_team_crc(const uint8_t rec[BATTLE_TEAM_MAX][BUGW_BYTES])
{
  ...
  return crc16_ccitt(&rec[0][0], (size_t)BATTLE_TEAM_MAX * BUGW_BYTES);
}
...
{
  c.wire_ver    = (uint8_t)BUGW_LAYOUT_VER;
  c.team_max    = (uint8_t)BATTLE_TEAM_MAX;
  c.level_max   = (uint8_t)ER_LEVEL_MAX;
...
VReject session_set_team(Session& s, const BugInstance* m, uint8_t count,
                         uint8_t& bad_index)
{
  bad_index = 0xFFu;
  memset(s.my_rec, 0, sizeof s.my_rec);
  ...
  // not Bug validity (game/validate.h), so it cannot live in the shared body
  // - and without it a player whose own team held a fainted Bug got VR_OK
...
static void on_team_submit(Session& s, const ProtoMsg& m)
{
  ...
  BugInstance them[BATTLE_TEAM_MAX];
  ...
  // BATTLE ELIGIBILITY. hp_cur == 0 was the last thing validate_bug(),
