// =============================================================================
//  ERRATA - networking/trade_link.h
...
//                validate_bug() (protocol.h), and then applies the POLICY
...
//                AND IT IS NOT NEGOTIABLE: nothing before it moves a Bug
...
// =============================================================================
#ifndef ER_NETWORKING_TRADE_LINK_H
#define ER_NETWORKING_TRADE_LINK_H
...
//  on - patching BUGW_OFF_ID and resealing the record's own CRC over bytes
//  0..BUGW_CRC_BYTES-1, so the patched record still decodes.
//
...
struct TradeHooks {
  ...
  uint8_t (*judge)(void* ctx, const uint8_t rec[BUGW_BYTES]);
  bool    (*journal_received)(void* ctx, const uint8_t rec[BUGW_BYTES]);    // W2
...
struct TradeLink {
  ...
  uint8_t  out_rec[BUGW_BYTES];   // ours, frozen at session_set_trade()
  uint8_t  in_rec[BUGW_BYTES];    // theirs, as received
  uint32_t out_id;               // the local id of the Bug we are giving
...
// and BEFORE session_start(); `out_id` is the local Bug id being given, and
...
// only thing left is the local player. The screen shows the two Bugs here.
...
#endif  // ER_NETWORKING_TRADE_LINK_H
