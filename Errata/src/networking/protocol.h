// =============================================================================
//  ERRATA - networking/protocol.h
//  THE SPEC SECTION 15 FRAME AND THE 48 B WIRE BUG (plan P4-C5).
//
...
//  own convention exactly: BugInstance.crc16 at 126 over 0..125,
...
// =============================================================================
#ifndef ER_NETWORKING_PROTOCOL_H
#define ER_NETWORKING_PROTOCOL_H
...
#include "../persistence/save_schema.h"    // BugInstance, TR_WIRE_BYTES
...
enum ProtoType : uint8_t {
  ...
  //     version byte.
  PT_TRADE_OFFER,         // 13  the 48 B wire Bug this device is giving
...
// The 48 B wire Bug - see section 3 below for why 48 and not 64.
#define BUGW_BYTES  48u
...
inline constexpr uint16_t PROTO_LEN_OF[(size_t)PT_TYPE_COUNT] = {
  ...
  12,                      // 4  SESSION_ACCEPT
  4 + 3 * BUGW_BYTES,       // 5  TEAM_SUBMIT   (148)
  8,                       // 6  TEAM_VALIDATION
  12,                      // 7  BATTLE_STATE
  8,                       // 8  ACTION
  8,                       // 9  ACTION_RESULT
...
// -----------------------------------------------------------------------------
//  3. THE WIRE BUG - 48 BYTES, AND 64 IS NOT AVAILABLE
//
...
//      cannot send. ONE BEHAVIOURAL CONSEQUENCE, STATED: a Bug may arrive
...
//      Bug that has never evolved, since game/xp.cpp only raises the bit on
//      a level-up.
...
//      RECEIVER'S.
//
//  P7 IS PRE-COMMITTED BY THIS AND CANNOT UNDO IT CHEAPLY: a traded Bug will
...
// -----------------------------------------------------------------------------
static_assert((int)BUGW_BYTES == (int)TR_WIRE_BYTES,
              "the wire Bug must fit the frozen PendingTrade.in_wire");

// NOT BUG_MAGIC: a 48 B wire record and a 128 B save blob must not answer to
// one magic, or a record pasted into the wrong slot would look plausible.
// 'P','W' - the pre-rename wire tag, kept so a peer that has not updated can
// still be talked to. See BUG_MAGIC in persistence/save_schema.h.
#define BUGW_MAGIC        0x5750u    // bytes 'P','W' little-endian
...
// either.
#define BUGW_LAYOUT_VER   1u

#define BUGW_OFF_MAGIC      0u   // 2
#define BUGW_OFF_WIRE_VER   2u   // 1
#define BUGW_OFF_SPECIES    3u   // 1
#define BUGW_OFF_ID         4u   // 4
#define BUGW_OFF_HP_CUR     8u   // 2
#define BUGW_OFF_XP        10u   // 2
#define BUGW_OFF_LEVEL     12u   // 1
#define BUGW_OFF_RESERVED0 13u   // 1   where evo_state would have gone
#define BUGW_OFF_STATUS    14u   // 1
#define BUGW_OFF_FLAGS     15u   // 1
#define BUGW_OFF_ORIGIN    16u   // 1
#define BUGW_OFF_TRAIT     17u   // 1
#define BUGW_OFF_MOVES     18u   // 4
#define BUGW_OFF_GENOME    22u   // 16, field by field, never a struct copy
#define BUGW_OFF_RESERVED  38u   // 8
#define BUGW_OFF_CRC       46u   // 2, over bytes 0..45
#define BUGW_CRC_BYTES     46u
static_assert(BUGW_OFF_CRC + 2u == BUGW_BYTES, "the wire Bug's CRC is not last");
static_assert(BUGW_CRC_BYTES == BUGW_BYTES - 2u, "the wire Bug's CRC span drifted");

// The status bits the WIRE may carry. Narrower than the stored mask on purpose,
// and the narrowing is owned HERE rather than by a flag inside validate_bug():
...
#define BUGW_STATUS_MASK  ((uint8_t)(PBS_SICK | PBS_ASLEEP))

// Writes the 48 B record. `p` is assumed already valid - this is the SEND side
// and a device only ever encodes its own Bugs. Total: it writes every one of
// the 48 bytes on every call, so no caller can leak a stale byte.
void pbw_encode(const BugInstance& p, uint8_t rec[BUGW_BYTES]);

// Reads one. THE DECODER PRODUCES A COMPLETE BugInstance, NOT A WIRE OBJECT:
// it fills a LOCAL, derives evo_state from the receiver's own tables, stamps
// BUG_MAGIC/BUG_LAYOUT_VER, and runs validate_bug() - then copies to
...
// persistence/save_manager.cpp's bug_seal() and this module does not link the
// persistence layer; whoever stores a received Bug seals it there. The wire
// record's own CRC is this layer's integrity check and it has already been
// applied by the time out is written.
//
// THE validate_bug() CALL IS REDUNDANT ON THE BATTLE PATH AND MUST STAY.
...
VReject pbw_decode(const uint8_t rec[BUGW_BYTES], BugInstance& out);
...
//  128 B BugInstances inside every ProtoMsg; instead the session layer calls
...
struct ProtoTeamSubmit   { uint16_t team_crc; uint8_t count;
                           uint8_t  rec[BATTLE_TEAM_MAX][BUGW_BYTES]; };
...
//     caller decodes it straight into the BugInstance it means to fill, so
...
//     record already carries its own CRC at BUGW_OFF_CRC over its own bytes, so
...
struct ProtoTradeOffer   { uint8_t  rec[BUGW_BYTES]; };
...
// record is a legal Bug and this device will not take it - a taint, a
...
uint16_t trade_rec_crc(const uint8_t rec[BUGW_BYTES]);
...
uint16_t trade_pair_crc(const uint8_t initiator_rec[BUGW_BYTES],
                        const uint8_t responder_rec[BUGW_BYTES]);
...
#endif  // ER_NETWORKING_PROTOCOL_H
