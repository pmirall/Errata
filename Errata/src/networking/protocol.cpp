// =============================================================================
//  ERRATA - networking/protocol.cpp
...
{
  memset(rec, 0, (size_t)BUGW_BYTES);          // every reserved byte, once
  put_u16(rec + BUGW_OFF_MAGIC, (uint16_t)BUGW_MAGIC);
  rec[BUGW_OFF_WIRE_VER] = (uint8_t)BUGW_LAYOUT_VER;
  rec[BUGW_OFF_SPECIES]  = p.species_id;
  put_u32(rec + BUGW_OFF_ID, p.id);
  put_u16(rec + BUGW_OFF_HP_CUR, p.hp_cur);
  put_u16(rec + BUGW_OFF_XP, p.xp);
  rec[BUGW_OFF_LEVEL]  = p.level;
  // BUGW_OFF_RESERVED0 stays 0: it is where evo_state would have gone, and the
  // receiver derives that field rather than believing it.
  rec[BUGW_OFF_STATUS] = (uint8_t)(p.status & (uint8_t)BUGW_STATUS_MASK);
  rec[BUGW_OFF_FLAGS]  = p.flags;
  rec[BUGW_OFF_ORIGIN] = p.origin;
  rec[BUGW_OFF_TRAIT]  = p.trait_id;
  for (uint8_t m = 0; m < (uint8_t)ER_MOVE_COUNT; ++m)
    rec[BUGW_OFF_MOVES + m] = p.moves[m];
  genome_put(rec + BUGW_OFF_GENOME, p.genome);
  put_u16(rec + BUGW_OFF_CRC, crc16_ccitt(rec, (size_t)BUGW_CRC_BYTES));
}
...
VReject pbw_decode(const uint8_t rec[BUGW_BYTES], BugInstance& out)
{
  ...
  //     and none of them can be produced by validate_bug().
  if (get_u16(rec + BUGW_OFF_MAGIC) != (uint16_t)BUGW_MAGIC) return VR_WIRE_MAGIC;
  if (rec[BUGW_OFF_WIRE_VER] != (uint8_t)BUGW_LAYOUT_VER)    return VR_WIRE_VERSION;
  if (get_u16(rec + BUGW_OFF_CRC) != crc16_ccitt(rec, (size_t)BUGW_CRC_BYTES))
    return VR_WIRE_CRC;
  if (rec[BUGW_OFF_RESERVED0] != 0u ||
      !all_zero(rec + BUGW_OFF_RESERVED, 8u))
    return VR_WIRE_RESERVED;

  const uint8_t species = rec[BUGW_OFF_SPECIES];
  const uint8_t flags   = rec[BUGW_OFF_FLAGS];
  ...
  if ((uint8_t)(rec[BUGW_OFF_STATUS] & (uint8_t)~BUGW_STATUS_MASK) != 0u)
    return VR_WIRE_STATUS_BITS;

  // --- BUILD A COMPLETE LOCAL INSTANCE. Never `out`: a caller must not be able
  //     to observe a half-trusted Bug, not even transiently.
  BugInstance loc;
  memset(&loc, 0, sizeof loc);
  loc.magic         = (uint16_t)BUG_MAGIC;
  loc.layout_ver    = (uint8_t)BUG_LAYOUT_VER;
  loc.species_id    = species;
  loc.id            = get_u32(rec + BUGW_OFF_ID);
  loc.hp_cur        = get_u16(rec + BUGW_OFF_HP_CUR);
  loc.xp            = get_u16(rec + BUGW_OFF_XP);
  loc.level         = rec[BUGW_OFF_LEVEL];
  loc.status        = rec[BUGW_OFF_STATUS];
  loc.flags         = flags;
  loc.origin        = rec[BUGW_OFF_ORIGIN];
  loc.trait_id      = rec[BUGW_OFF_TRAIT];
  for (uint8_t m = 0; m < (uint8_t)ER_MOVE_COUNT; ++m)
    loc.moves[m] = rec[BUGW_OFF_MOVES + m];
  genome_get(rec + BUGW_OFF_GENOME, loc.genome);
  ...
  // "there is none" value persistence/save_manager.cpp's bug_clear() writes.
  loc.custom_sprite = (uint8_t)ER_CUSTOM_SPRITE_NONE;
  ...
  // species leaves it 0 and validate_bug() names the real problem one line
  // below.
  {
  ...
  const VReject r = validate_bug(loc);
...
{
  return get_u16(rec + BUGW_OFF_CRC);
}
                        ...
                        const uint8_t responder_rec[BUGW_BYTES])
{
  ...
  uint8_t both[(size_t)BUGW_BYTES * 2u];
  memcpy(both, initiator_rec, (size_t)BUGW_BYTES);
  memcpy(both + BUGW_BYTES, responder_rec, (size_t)BUGW_BYTES);
...
ProtoErr proto_encode(const ProtoMsg& m, uint8_t* buf, size_t cap, size_t& n_out)
{
  ...
  // moment before a Bug is on the wire at which the two can still disagree
  ...
  switch ((ProtoType)m.type) {
    ...
    case PT_TEAM_SUBMIT:
      ...
      for (uint8_t i = 0; i < m.p.team.count; ++i)
        memcpy(q + 4 + (size_t)i * BUGW_BYTES, m.p.team.rec[i], (size_t)BUGW_BYTES);
      break;
    ...
    case PT_GOODBYE:
      ...
      break;
    case PT_TRADE_OFFER:
      memcpy(q + 4, m.p.toffer.rec, (size_t)BUGW_BYTES);
  ...
  switch ((ProtoType)type) {
    ...
    case PT_TEAM_SUBMIT: {
      ...
      for (uint8_t i = count; i < (uint8_t)BATTLE_TEAM_MAX; ++i)
        if (!all_zero(q + 4 + (size_t)i * BUGW_BYTES, (size_t)BUGW_BYTES))
          return PE_TEAM_PAD;
      m.p.team.count    = count;
      m.p.team.team_crc = get_u16(q + 2);
      // THE RECORDS STAY RAW. pbw_decode() is the caller's to run, straight into
      // whatever BugInstance it means to fill - see protocol.h.
      memcpy(m.p.team.rec, q + 4, (size_t)BATTLE_TEAM_MAX * BUGW_BYTES);
      break;
    }
    case PT_TEAM_VALIDATION:
    ...
    case PT_TRADE_OFFER:
      ...
      memcpy(m.p.toffer.rec, q + 4, (size_t)BUGW_BYTES);
      break;
    case PT_TRADE_READY:
