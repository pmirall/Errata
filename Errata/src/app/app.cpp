...
//
//  Errata.ino is a three line shim that forwards setup()/loop() here, so the
...
// =============================================================================
//  MODULE-OWNED STATE
//  The live Bugs belong to persistence/game_state.cpp and the active one is
...
//
//  game_state calls this at the instant it commits a Bug, so the snapshot is
...
//  theirs - clamped at ER_CARE_MILLI_MAX, never above, on the ACTIVE Bug
...
void app_pay_activity(void)
{
  ...
  // in between. A BUG AT XP_LEVEL_MAX USED TO BE THE ONE CASE WHERE THIS PAID
  ...
  if (happy != 0u) {
    const uint8_t act = box_active();
    BugInstance* p = (act < (uint8_t)BOX_SLOTS) ? box_slot(act) : nullptr;
    if (p != nullptr && p->care[CARE_HAPPINESS] < (int32_t)ER_CARE_MILLI_MAX) {
...
//
//  THE CONTEXT IS BUILT HERE because this is where a BugInstance can be
...
// =============================================================================
static void evo_context_of(const BugInstance& p, EvoContext& ctx)
{
  evo_context_clear(ctx);

  int32_t happy = p.care[CARE_HAPPINESS];
  if (happy < 0)                  happy = 0;
  if (happy > ER_CARE_MILLI_MAX)  happy = ER_CARE_MILLI_MAX;
  ctx.happiness = (uint16_t)(happy / (ER_CARE_MILLI_MAX / 100L));   // milli -> %
...
bool app_evolution_offer(void)
{
  ...
  const uint8_t act = box_active();
  if (act >= (uint8_t)BOX_SLOTS) return false;
  const BugInstance* p = box_peek(act);
  if (!p) return false;
...
bool app_evolve_active(void)
{
  ...
  // back and refuse instead: the player sees no ceremony, keeps their bug,
...
// =============================================================================
//  BOOT: THE BOX AND THE ACTIVE BUG
//  persistence decides whether there is anything to load; game/box.cpp decides
//  which slot is active and what a brand new Bug is; the app.cpp only
//  sequences them.
//
//  NEVER an auto-wipe. A load that refused to touch flash (LOAD_CORRUPT,
//  LOAD_FOREIGN_NEWER) leaves the session read-only: the placeholder Bug
...
static bool app_trade_write_slot(void* ctx, uint8_t slot)
{
  ...
  // save_bug_now(), NOT save_bug(..., true) - and this shim is the BOOT
  ...
  return save_bug_now(slot, gs_state().bugs[slot]);
}
...
//  the next boot leaves the Box holding either the outgoing Bug or the
...
//  boot_box(), because the resolver can change WHICH Bug is in the Box and
...
static void boot_trade(void)
{
  ...
  // The player is told. A Bug that silently appeared or silently went is the
...
static void boot_box(void)
{
  ...
  // On a read-only session this Bug is never written; it exists only so the
  ...
  // refill of a spent one - and a Bug out of the box should not owe its
  // owner six minutes before the first care action is worth anything.
  xp_ledger_reset(1);

  const uint32_t now  = gt_now();
  const uint8_t  slot = box_new_bug((uint8_t)SPECIES_ID_STARTER, 1,
...
//  not in ui.cpp because recovery ends with a DIFFERENT Bug in the Box, and
...
static void boot_absence(void)
{
  ...
  // The nine stored Bugs do not live: they only recover, each from its own
...
void app_setup(void)
{
  ...
  // --- entropy --------------------------------------------------------------
  // The ONE esp_random() call of the firmware (plan Â§1.4): it seeds every
  ...
  // to be before boot_box(), because the resolver can change which Bug the
...
static void logic_tick(uint32_t owed)
{
  ...
  // Bug that is awake and switched on earns for being carried. Both happen
  ...
  //
  // WHOLE BOX, NOT THE ACTIVE SLOT. A benched Bug's day passes at the same
  ...
  // stored Bug; the status simply stays until the session ends. The status
  // and the deadline live in the Bug record, so the change is persisted by
  // gs_save_active() below on the active slot - and by save_bug_now() when a
  ...
  if (!gs_readonly() && cor_service(gs_state().bugs, (uint8_t)BOX_SLOTS,
