...
static void link_trend_service(uint32_t now_ms)
{
  ...
  // acceptance step that had no instrument. ERRATA_IMPLEMENTATION_PLAN.md's
...
static bool run_wipe(void)
{
  ...
  // BugInstance the simulation is still bound to and mints NO SLOT - it is
  // not a Box constructor (game/box.h calls box_new_bug() "THE TREE'S ONE
...
// copy by one byte and requires bug_blob_ok() to refuse it. The mutation is
...
static void run_save_load(DiagOut& o)
{
  ...
  const BugInstance* live = (slot < (uint8_t)BOX_SLOTS) ? box_peek(slot) : nullptr;
  if (!live) { diag_puts(o, "test_save_load: no active slot\n"); return; }

  BugInstance copy = *live;
  bug_seal(copy);
  const bool sealed_ok = bug_blob_ok(copy);

  BugInstance bad = copy;
  bad.level = (uint8_t)(bad.level ^ 0x01u);       // one bit, anywhere under the CRC
  const bool corrupt_refused = !bug_blob_ok(bad);

  const uint16_t fails_before = kv_write_fails();
  const bool wrote  = gs_save_active(true);
  const bool landed = save_bug_landed();
...
static bool shell_dev_line(const char* line)
{
  ...
  if (r == (uint8_t)DGR_OK) {
    ...
    Serial.printf("[diag] %s: no active Bug\r\n", sp->name);
  }
  return true;
}
...
static void draw_sys(void)
{
  ...
  switch (s_sys_page) {
    ...
    // this phase - build, save version, Bug count, last scan, protocol -
    ...
    case GD_SYS_INFO: {
      ...
      snprintf(b, sizeof(b), "bugs %u/%u  proto %u  batt n/a",
               (unsigned)f.bugs, (unsigned)f.box_cap,
