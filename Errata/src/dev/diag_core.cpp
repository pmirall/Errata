...
static constexpr DiagCmdSpec kCmds[DGC_COUNT] = {
  ...
  { "spawn",          DGC_SPAWN,          1, 2, DCF_MUTATES | DCF_TAINTS,
    "spawn <species> [level] - file a new Bug in the Box" },
  { "set_level",      DGC_SET_LEVEL,      1, 1, DCF_MUTATES | DCF_TAINTS,
    "set_level <n> - set the active Bug's level, 1..30" },
  ...
  // arm touches no BugInstance itself. What it does is note interactions
  // that app/app.cpp will LATER pay as XP_SRC_CARRY onto the active Bug
  ...
  // Bug is marked here, where the decision is made.
  ...
  { "test_evolution", DGC_TEST_EVOLUTION, 0, 0, DCF_MUTATES | DCF_TAINTS,
    "test_evolution - force the active Bug's evolution rule" },
...
void diag_taint(BugInstance& p)
{
  ...
  // different writers on different paths and bug_is_tainted() reads both.
...
//  5. EXECUTION
// =============================================================================
static BugInstance* active_bug(const DiagCtx& ctx)
{
...
// A genome for a Bug minted inside the console. genome_genesis() CLEARS the
...
// Files one Bug and marks it. THE ONE PLACE a spawn is written, so
...
static uint8_t mint_one(const DiagCtx& ctx, uint8_t species, uint8_t level)
{
  ...
  const uint8_t slot = box_new_bug(species, level, (uint8_t)ORIGIN_WILD, g,
                     ...
                     const BugInstance& p)
{
  ...
  diag_puts(o, " tainted=");diag_putu(o, bug_is_tainted(p) ? 1u : 0u);
  diag_nl(o);
}
                  ...
                  uint8_t& commit)
{
  ...
  switch (p.cmd) {
    ...
    case DGC_SPAWN: {
      ...
      if (lv < 1u || lv > (uint32_t)ER_LEVEL_MAX) {
        diag_puts(out, "level 1..");
        diag_putu(out, (uint32_t)ER_LEVEL_MAX);
        diag_nl(out);
        return (uint8_t)DGR_ERR_RANGE;
      }
    ...
    case DGC_SET_LEVEL: {
      ...
      // by hand with hp_cur left where it was is a Bug whose own numbers
    ...
    case DGC_SET_ACTIVITY: {
      ...
      // the app to pay through the normal capped path, and the Bug that will
    ...
    case DGC_HEAL: {
      ...
      diag_puts(out, " tainted=");  diag_putu(out, bug_is_tainted(*a) ? 1u : 0u);
      diag_nl(out);
      commit = DGCOMMIT_ACTIVE;
      return (uint8_t)DGR_OK;
    }
    ...
    case DGC_TEST_ENCOUNTER: {
      ...
      in.category     = 0u;
      in.rssi         = -60;
      const BugInstance* a = active_bug(ctx);
    ...
    case DGC_TEST_EVOLUTION: {
      ...
      // Bug past its rule: evolution_apply() re-checks evolution_ready() and
      ...
      // The rule's own level, not ER_LEVEL_MAX: the point is to exercise the
      ...
      // either way, and a Bug this arm touched is not a clean one.
      diag_taint(*a);
...
static const char* const kFieldName[DGD_FIELD_COUNT] = {
  ...
  "Clock", "Save version", "Bug count", "Last scan", "Last error",
  "Protocol version"
};
...
static const char* const kScreenName[] = {
  ...
  "STATUS", "STATUS_B", "NETWORK", "LINK", "CREATOR", "MANUAL", "SETTINGS", "TIME",
...
void diag_fmt_field(uint8_t id, const DiagFields& f, DiagOut& o)
{
  ...
  switch (id) {
    ...
    case DGD_BATTERY:
      ...
      // and ER_PINS_CONFIRMED is never defined. Printing a fabricated
      // percentage would be the single most misleading thing this console
      // could do.
      diag_puts(o, "n/a (no divider fitted; D10 open, ER_PINS_CONFIRMED unset)");
      break;
