...
//  legacy/mkfixtures.cpp) are byte images of the v1 PetSave / Config /
...
TEST(fixture_petsave_v1_adult) {
  ...
  CHECK_EQ(u16_at(offsetof(PetSave, crc16)), 0xD65E);      // stored crc16 field
...
TEST(fixture_petsave_v1_egg) {
  ...
  CHECK_EQ(p.genome.crc16, crc16_ccitt(&p.genome, GENOME_CRC_BYTES));
  CHECK_EQ(p.crc16, crc16_ccitt(&p, PETSAVE_CRC_BYTES));
  CHECK_EQ(u16_at(offsetof(PetSave, crc16)), 0xFE27);
