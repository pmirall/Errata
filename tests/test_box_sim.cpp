// =============================================================================
//  Errata host tests - test_box_sim.cpp
//  game/box_sim.cpp: the ONE place that knows both that the Box stores Bugs
...
//  active slot leaves sim bound to the OTHER Bug - the player keeps petting,
...
// A bound Box holding `n` Bugs, each one a CHILD (level 5) so the stage is
...
TEST(swapping_the_active_slot_keeps_the_simulation_on_the_same_creature) {
  ...
  // it back to 50 %) and hunger lives in the BugInstance.
  ...
  CHECK(sim_bug() == box_slot(box_active()));
  CHECK_EQ((unsigned)sim_bug()->id, (unsigned)carried);

  // A rebind is an address correction, NOT a sim_switch(): not one per-Bug
...
// Bug up the list. The binding has to follow it there too.
...
TEST(sim_rebind_is_not_sim_switch) {
  ...
  CHECK(sim_bug() == box_slot(1));
  CHECK_EQ((int)sim_stat_pct(ST_BOND), 90);

  sim_switch(*box_slot(2));
  CHECK(sim_bug() == box_slot(2));
