...
- [ ] Re-check the soak criterion now that **D13 is closed** (P3-C2b). "No stat pinned at 0 for more than 6 simulated hours of neglect" used to be unreachable, because a pebble whose light nobody turned off never slept and energy pinned at 0 for ever. Since P3-C2b it sleeps every night and wakes recharged (`care_a_full_night_of_neglect_is_spent_asleep_and_recharging` pins that), so ENERGY can now satisfy the criterion — but HUNGER and HAPPINESS still cannot: at 4,200 and 3,000 milli/h they empty in about a day of total neglect and stay at 0, which is the design (health is what the floor protects). So restate the criterion as "no stat that the player is not deliberately ignoring", or scope it to energy and health, and run it.
