...
- [ ] **Carried forward from P3-C3 (do not lose these).** The generated content already exists and is verified in the session scratchpad (`content/final/*.json`, 60 species / 34 attacks / 40 rules); its own `verify.py` is the gate for it. Six obligations attach to this step:
  ...
  6. **Make the evolution VISIBLE (added by P3-C5, see the open bullet under P3-C3).** `pet_view_fill_sim()` is the path the firmware runs and it never sees `species_id`: `form` comes from the genome, the minor form and the life stage, none of which an evolution moves, and `sprite_lookup_pose()` keys on `gene_species(genome)` rather than the species row. Growing the roster to twelve makes this louder, not quieter — twelve species that all wear their genome's body. The view has to learn `species_id`, `SpeciesDef.sprite_id` has to be read by something, and the species NAME has to reach a screen (HOME shows the genome dynasty name today). `pet_view_fill()`, which does read `evo_state`'s stage bits, is dead code the tests drive; either wire it up or delete it, but do not leave a tested function the device does not call.
