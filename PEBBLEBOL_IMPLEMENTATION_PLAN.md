...
4. Grep gates (added as the corresponding commits land): `grep -rE "s_screen\s*=" src/` == 0 outside `app/state_machine.cpp` (P2-C11 exit, **tree-wide since P2-C11d**: the exception this gate used to carry - `dev/godmode.cpp`'s unrelated `static uint8_t s_screen` for its console sub-screen - was removed by renaming that variable `s_console_page`); `grep -c "nullptr," src/ui/screen_table.cpp` == 0 (P2-C11d: every §6 state has a row with all five hooks); `grep -c "WiFi.begin" src/` == 0 (P5-C1); `grep -c "genome_rand\|esp_random" src/game src/minigames` == 0 (P2-C2/P3-C4); `grep -c "rd_u8g2()" src/app src/minigames src/game` == 0 (P10-C4).
...
- [x] New logic files for the §29 set: `packet_flood`, `firewall`, `buffer`, `delete`; each 5-15 s, fixed 25 ms step (ui.cpp:1731-1739 pattern), per-run `Rng`, drawing only through `gfx_*`. **P3-C4b.** Runs are 11,675 / 12,000 / 10,000 / 11,600 ms and every one of them is a CONSTANT rather than a worst case — no press can lengthen a run in any of the four, which is the hole both shipped games had. The two collapse risks the step was scoped around are now compiled assertions rather than prose: FIREWALL is discrete (`c.score == fw_blocked(c) * 125` asserted after every press and every step, and a tape that sits in the correct lane for four fifths of every fall and steps out just before impact scores exactly 0), BUFFER is continuous (`c.score == buf_inside_steps(c) * 1000 / 340`, same per-step check, and a press is an impulse rather than a move); DELETE requires choosing WHICH block (a tape that walks to the nearest block of either kind must score strictly less than one that reads its type, and purging a healthy block is checked directly to pay nothing), and PACKET_FLOOD is a sort (its tape routes by matching the packet's digit against the two mouth digits and nothing else). Three small hardenings came out of review, all in the same shape — an arithmetic claim that lived in a comment now lives in the compiler: PING gained the run-budget `static_assert` the other five already had (its header said 5 * (1999 + 900) = 14,495 ms, but a round ends at the first step STRICTLY past arm + window and elapsed time is always a whole number of steps, so the bound is 5 * 2900 = 14,500 — the thinnest margin of the six); DELETE's choice proof now DERIVES its "3 indices apart" from `DEL_LIFE_MS`/`DEL_SPAWN_MS` instead of spelling the 3, because a hardcoded 3 keeps passing when the fuse is shortened, which is the one edit that kills the property; and `packet_flood_draw.cpp` stopped pulling `<stdio.h>` into a device translation unit to `snprintf` a single decimal digit. Three framework defects were fixed on the way, each of which would have bitten these four: `MgCtx::state` sat at offset 14 and `mg_state<T>()`'s `alignof` assert cannot see an offset, so every 32-bit game field was being read at 2 mod 4 (`alignas(uint32_t)` plus an `offsetof` assert; `sizeof(MgCtx)` stays 48); `draw_str_list()` sized its row array `CARE_ROWS` (5) and clamped, so at `PLAY_ROWS = 7` BORRAR and "Volver" would never have drawn while the cursor still rang through them; and the in-run pause was on `GST_TAP_R`, the same physical press `game_service()` feeds to `mgr_press()`, so one B press both played the game and opened a modal over it — moved to `GST_HOLD_R`, which emits no tap on release, with the strip now saying `JUGAR` / `JUGAR/PAUSA` during a run instead of advertising B as PAUSA in six games that use it to play. **The second half of that one landed in review**: moving the gesture stopped the collision but not the run — `sm_service()` calls the GAME update hook whatever is on top of the screen, so the confirm went up while the game kept stepping, kept scoring and kept eating the raw press edges the player was answering the dialog with. `game_service()` now returns early on `dialog_modal() != MODAL_NONE`, after moving its own timestamp forward so the paused time is dropped rather than delivered as one huge `dt`, and drains both press edges so the presses spent on the dialog are not replayed into the game.
...
- [x] Matrix + tag `v0.3.0-pet`; `test_sim_golden` renamed `test_care_golden`; bench: god-mode x3600 soak (godmode.cpp:538-597 CSV) shows no stat pinned at 0 for > 6 simulated hours of neglect. **The bench moved into the suite instead of the god console.** The criterion above is a host test that runs in about a millisecond, so it is a GATE rather than a thing somebody has to remember to drive on a device that has never been flashed; the god-mode CSV stays as the on-hardware confirmation and is listed with the other first-flash measurements. `test_sim_golden.cpp` → `test_care_golden.cpp` and `golden/sim_v1.txt` → `golden/care_v2.txt` (`git mv`, `tests/Makefile` and both `retune:` targets updated): the `v1` meant the legacy v1 SIMULATION the file was first recorded from, which two retunes later is no longer what it pins. Only the transcript's header line changed; every hashed minute is byte-identical, which the diff and the passing test prove together. Matrix green on all 7 variants at 0 project warnings, flash 1,893,072 of 2,400,000 (+32 B for the poop carry), globals 70,348 of 90,000.
