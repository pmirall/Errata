...
|---|---|---|---|---|
| **D1** | **GPIO map (pin conflict, unresolved).** `config.h:92-96` compiles `PIN_SDA 8, PIN_SCL 9, PIN_BTN_L 10, PIN_BTN_R 2, PIN_LED 5` with the human comments "tu cableado actual del TinyLLM" and "OBLIGATORIO cambiarlo: 8 ya es SDA"; config.h:89-90 itself says "GPIO2 and GPIO9 are strapping pins: never wire a button to them. GPIO8 = LED_BUILTIN". README.md:60-67/:71/:73-85, CHANGELOG.md:170, render.h:29-31, render.cpp:14-18 all say SDA=6, SCL=7, BTN_L=3, BTN_R=4, LED=8. As compiled, SCL sits on the boot strap (GPIO9 low at boot = download mode), a button on GPIO2 (must be high at boot), and `rd_fatal()` blinks GPIO5 where no LED exists. Nothing has ever run on hardware. Extra input for the decision: on the ESP32-C3 only GPIO0-5 can wake the chip from **deep** sleep, so the documented map (buttons 3/4) permits Phase 6 deep sleep, the compiled map (10/2) permits light sleep only. | audit §18 risk 1 | P2-C0 first flash (hardware track only; no host work blocks on it); P6-C3 deep sleep | **Keep `config.h` values untouched.** P2-C8 wraps the five pins in one `// DECISION D1 PENDING` block in `src/core/config.h` and adds `static_assert`s that fire if `PIN_SDA == PIN_LED` or any button sits on GPIO 2/8/9 **only once `PB_PINS_CONFIRMED` is defined**, so nothing changes before the owner inspects the board. The fix is then a 5-line commit. |
| **D2** | **Peer-session transport: BLE vs Wi-Fi (ESP-NOW).** The legacy advert carries 19 B/frame (ble_social.cpp:62-91, config.h:546-550) and cannot carry §15; GATT was rejected by the original author for stability (ble_social.cpp:8-25); each BLE bring-up burns one of 32 sessions with a claimed ~672 B Bluedroid leak (config.h:559, net.cpp:196-204, unmeasured); BLE costs 721,632 B flash / 23,688 B static RAM (audit §14). ESP-NOW is in the core (250 B/frame, unicast + ACK) and needs `WIFI_STA` residency — the same residency the §40 scanner already needs (`WiFiScan.cpp:75` forces `enableSTA`), so no stack flip and no Bluedroid heap (~70 KB). Neither has run on hardware. | audit §7.3, §19.2 | P7-C1 | **Recommendation: ESP-NOW for discovery beacons and sessions** behind the §59 `Transport` interface, `FEATURE_BLE 0` in the release build. Documented fallback (option B, from the reuse draft): BLE legacy advert = discovery beacon v2 + a 19 B `INVITE` frame `{proto_ver, device_id u32, caps u8, op u8, session u16, channel u8, name_hash u16, crc u16}`; on accept both sides flip to `RADIO_WIFI` (ESP-NOW on the channel named in the INVITE) — solves the ESP-NOW same-channel constraint at the cost of keeping BLE resident (721,632 B). `ble_social.cpp` plumbing stays compiled behind `FEATURE_BLE` until D2 is closed. P7-C1 measures the BLE init/deinit leak (32 cycles) if BLE is kept. |
...
Decisions live in `docs/decisions.md` (created P1-C1); each closed decision is one commit that also updates that file.
...
4. Grep gates (added as the corresponding commits land): `grep -cE "s_screen\s*=" src/` == 0 outside `state_machine.cpp` (P2-C11 exit); `grep -c "WiFi.begin" src/` == 0 (P5-C1); `grep -c "genome_rand\|esp_random" src/game src/minigames` == 0 (P2-C2/P3-C4); `grep -c "rd_u8g2()" src/app src/minigames src/game` == 0 (P10-C4).
...
| `networking/creator_server.h` (ex webui) | keep `web_begin/web_service/web_stop/web_running`, `rate_take` (webui.cpp:270-285), `pin_ok` (:352-364), `arg_u32` (:173-184), 4-arg `send_P` (:573-581), phase gate (:1088-1094); routes exactly `GET /`, `GET /api/state`, `GET /api/schema`, `POST /api/validate`, `POST /api/pebble`, `POST /api/time`, `POST /api/ping` (§38); body cap: register POST routes with the 4-arg `on(uri, HTTP_POST, fn, rawfn)` overload — `FunctionRequestHandler::canRaw()` (core `RequestHandlersImpl.h:97`) routes the body through `raw()` in `HTTP_RAW_BUFLEN` chunks (core `Parsing.cpp:182-193`) instead of `readBytesWithTimeout()` mallocing `Content-Length` bytes (`Parsing.cpp:44`); at `RAW_START` the handler checks `server.clientContentLength()` (WebServer.h:196) `> CS_BODY_MAX (2048)` → discard + 413; `uint32_t cs_idle_ms(void)`; `bool cs_take_upload(PebbleInstance&, CustomSpeciesRec&)`; `bool cs_take_time(uint32_t&)` | audit §12 "Body limits: none" | P8-C3 |
...
- [ ] `tests/Makefile`, `tests/nt_test.h` (static registry, `TEST`, `CHECK`, `CHECK_EQ`, `CHECK_NEAR`, file:line on failure, non-zero exit; `main()` accepts `--seed N` and `--filter`), `tests/host_shims.cpp` providing `nt_input_test_millis/nt_input_test_level` (input.cpp:45-46) and `gt_host_millis32` (gametime.cpp:28).
- [ ] Extract `core/crc16.{h,cpp}` from storage.cpp:140-156; make `store_crc16`, `genome_crc16` and the BLE copy (ble_social.cpp:171-183) call it. `test_crc16.cpp` (`"123456789"` → 0x29B1, genome.h:54).
- [ ] Extract `core/rng.{h,cpp}` from genome.cpp:28-45 with the `RngStream` enum of §1.4; `genome_rand()` → `rng_u32(RNG_BREEDING)`; sim's `g_rng/rnd()` (sim.cpp:92, :154-160) → `RNG_CARE`; `sim_seed()`/`genome_set_rng()` kept as wrappers; `esp_random()` called once in the `.ino` (`rng_seed_all`), removing the calls at .ino:365-366, storage, webui, ble_social. `test_rng.cpp` (determinism per stream, independence, `rng_below` bounds, seed 0 remapped).
- [ ] `test_sim_golden.cpp`: fixed seeds, fixed `SimEnv`, scripted 6 h (feed hourly, clean at 2 h, play at 3 h), hash of `stat[]` + `flags` every 60 s → `tests/golden/sim_v1.txt`. Record from the **pre-change** sim (build the test first, then switch CRC/RNG callers — hash must still match).
- [ ] `test_genome.cpp` (genesis/seal/validate/breed determinism/hex32 round trip), `test_input.cpp` (waveforms through the FSM input.cpp:131-216: tap, hold-repeat, both, long-both, boot-held swallow :307-319, no phantom TAP before BOTH), `test_qr.cpp` (v1-v4 capacities 17/32/53/78 B, known vector), `test_clock.cpp` (`gt_format_elapsed` cases, millis-wrap with `GT_HOST_NEVER_VALID`).
- [ ] `tests/legacy/mkfixtures.cpp`: a host program linking the legacy `PetSave`/`Config`/`GainSave` structs (nt_types.h) that writes `tests/fixtures/petsave_v1_adult.bin`, `petsave_v1_egg.bin`, `config_v1.bin`, `gainsave_v1.bin`; fixtures committed; `test_fixtures.cpp` asserts their CRCs.
...
- [ ] Execute §2 G4: delete `wifi_wanted/radio_policy` (.ino:182-226), `NT_WIFI_RETRY_MS`, portal→STA retry (net.cpp:574-586), `CF_WEB_ENABLED` default (storage.cpp:801-803). Radio requests become screen-owned: SCR_QR requests `RADIO_WIFI` on enter (ui.cpp:3111) — add the release in `screen_leave` (ui.cpp:3129).
- [ ] `gametime`: add `gt_set_epoch(epoch, src)` + `gt_cal_state()` with the §1.7 rollback guard; `gt_is_valid()` (gametime.cpp:266-278) becomes true after a set; delete `gt_sync_start` and `SNTP_*` (config.h:460-467); `time()` stays the RTC-backed source.
- [ ] Minimal on-device time entry screen (year/month/day/hour/minute; L = next field, R = increment, hold = confirm) reachable from SETTINGS; calls `gt_set_epoch(..., CAL_USER)` + `store_touch_lastseen`; also the §26 first-boot "ask for time during setup" step.
- [ ] Absence on boot (.ino:304-333): **unknown clock charges zero** — `sim_catch_up_ex` no longer applies the `ABSENCE_LARGA_S` floor (sim.cpp:1920-1930; config.h:428 deleted); `ABS_UNKNOWN` retro-fix triggers on `gt_set_epoch` instead of SNTP landing (.ino:438-460).
- [ ] Replace `settle()`'s `delay(RADIO_SETTLE_MS)` (net.cpp:142-144) by a timed `NPH_SETTLING` phase pumped by `net_service()`.
- [ ] `input.cpp`: 5 ms `esp_timer` sampler → 16-deep SPSC ring `{ms, levels}` → `input_poll()` drains into the unchanged FSM; `INPUT_POLL_MS` (config.h:168) is now real; host shims unchanged. DIAG/god command `stall <ms>` (busy-waits in `loop()`) to prove presses during a 300 ms stall are still recognised.
- [ ] `test_clock.cpp` += set-epoch, rollback refused (> 5 min backwards unless CAL_USER), forward jump accepted, `CAL_UNSET` → zero absence; new `test_overflow.cpp`: `now < last_seen` → 0, `now − last_seen > ABSENCE_MAX_S` → unknown, u32 saturating adds, millis wrap.
...
- [ ] Step 1 (mechanical): wrap sim.cpp's 48 file-scope statics (sim.cpp:89-149) into `struct CareCtx` + one `static CareCtx g;` — golden must match.
- [ ] Step 2: `sim_bind(PebbleInstance&)` replaces `sim_init(PetSave&)`; `PetSave` deleted from live code (kept in `legacy_v1.h`); field map `stat[]`→`care[]`, `stat_rem[]`→`care_rem[]`, `flags`→`status/flags`, `last_seen_epoch`→`last_updated_epoch`; golden re-derived through the map (same numbers, new field names) — no `retune:`.
- [ ] `sim_switch(PebbleInstance& next)`: split `reset_ram_state(fresh)` (sim.cpp:1340) into per-Pebble accumulators (reset) and the device-wide gain ledger (kept), so changing the active slot cannot be used to farm.
- [ ] `game/box.{h,cpp}` + `test_box.cpp` (capacity 10, unique ids, exactly one active, swap, release requires explicit flag and refuses the active, recover caps at 100 %, never touches xp/level); `box_recover()` initial rule = the regen rule (sim.cpp:920-935) generalised at a stored rate.
- [ ] Boot: `box_recover(elapsed)` for every stored slot (computed from `last_updated_epoch`, written only on mutation), `sim_catch_up_ex` for the active (replaces `boot_absence`, .ino:304-333).
- [ ] First-boot starter: `pebble_new(species 1, level 1, origin STARTER)` into slot 0 (replaces `sim_new_pet(genome_genesis())`, .ino:283); onboarding polish in P10-C4.
- [ ] Catch-up budget: DIAG/god command `catchup <days>` reports µs for the worst case (400 d); if > 1 s, make `sim_catch_up_ex` resumable (≤ 200 steps per call) and pump it from LOAD_SAVE with a progress bar (§1.7).
...
- [ ] `ui/pet_view.{h,cpp}` (§1.4, with `identity` from `pebble_identity`); `petfx.cpp` stops including `sim.h`/`genome.h` (petfx.cpp:26-28; reads :580, :764-770, :894, :965); `actfx_begin(action, const PetView&)` (actfx.cpp:588); `web_pose_of/web_mood_index` (ui.cpp:348, :920) move into `pet_view.cpp`.
...
**P8-C1 PIN state** — S
- [ ] `ConfigV2.creator_pin` generated at first CREATOR entry (random 4 digits, shown on the device only), `pin_fail_count` + `pin_lock_until` (5 failures → 60 s); `pin_ok()` (webui.cpp:352-364) reads it from an `X-Pin` header or JSON field; PIN removed from the QR payload (`net_url`, net.cpp:653-671; §39).
...
| 25 | **Task WDT 5 s vs offline catch-up in `setup()`** (2,000 steps x ~100 soft 64-bit divisions) | Low (estimated ≈ 0.25 s; unmeasured) | P2-C10 measures the 400 d worst case; resumable catch-up pumped from LOAD_SAVE if > 1 s |
